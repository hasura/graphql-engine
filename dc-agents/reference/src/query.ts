...
const prettyPrintBinaryComparisonOperator = (
...
const prettyPrintBinaryArrayComparisonOperator = (
...
const prettyPrintUnaryComparisonOperator = (
...
const dateTimeSameDayAs = (a: RawScalarValue, b: RawScalarValue): boolean => {
  if (typeof a !== 'string') return expectedString(typeof a);

  if (typeof b !== 'string') return expectedString(typeof b);
...
const dateTimeInYear = (a: RawScalarValue, b: RawScalarValue): boolean => {
  ...
  if (typeof b !== 'number') return expectedNumber(typeof b);

  const bString = b.toString();
  return bString.length === 4 && a.startsWith(bString);
};

const getBinaryComparisonOperatorEvaluator = (
  operator: BinaryComparisonOperator,
): ((left: RawScalarValue, right: RawScalarValue) => boolean) => {
  switch (operator) {
    case 'greater_than':
    ...
    case 'equal':
      return (a, b) => a !== null && b !== null && a === b;
    case 'same_day_as':
      return dateTimeSameDayAs;
    case 'in_year':
      return dateTimeInYear;
    default:
      return unknownOperator(operator);
  }
};

const getBinaryArrayComparisonOperatorEvaluator = (
  operator: BinaryArrayComparisonOperator,
): ((left: RawScalarValue, right: RawScalarValue[]) => boolean) => {
  switch (operator) {
    case 'in':
      return (a, bs) => a !== null && bs.includes(a);
    default:
      return unknownOperator(operator);
  }
};

const getUnaryComparisonOperatorEvaluator = (
  operator: UnaryComparisonOperator,
): ((value: RawScalarValue) => boolean) => {
  switch (operator) {
    case 'is_null':
      return (v) => v === null;
    default:
...
const prettyPrintComparisonColumn = (
  comparisonColumn: ComparisonColumn,
): string => {
  return (comparisonColumn.path ?? [])
    .concat(comparisonColumn.name)
    .map((p) => `[${p}]`)
    .join('.');
};

const prettyPrintComparisonValue = (
  ...
  switch (comparisonValue.type) {
    ...
    case 'scalar':
      return comparisonValue.value === null
        ? 'null'
        : comparisonValue.value.toString();
    default:
...
const makeFilterPredicateBuilder =
  (
    getComparisonColumnValue: (
      comparisonColumn: ComparisonColumn,
      row: Record<string, RawScalarValue>,
    ) => RawScalarValue,
    performExistsSubquery: (
      exists: ExistsExpression,
      row: Record<string, RawScalarValue>,
    ) => boolean,
  ) =>
  (expression: Expression | null) =>
  (row: Record<string, RawScalarValue>): boolean => {
    const extractComparisonValueScalar = (
      comparisonValue: ComparisonValue,
    ): RawScalarValue => {
      switch (comparisonValue.type) {
        case 'column':
    ...
    const evaluate = (e: Expression): boolean => {
      switch (e.type) {
        ...
        case 'binary_op':
          ...
          return getBinaryComparisonOperatorEvaluator(e.operator)(
            binOpColumnVal,
            binOpComparisonVal,
          );
        case 'binary_arr_op':
          const inColumnVal = getComparisonColumnValue(e.column, row);
          return getBinaryArrayComparisonOperatorEvaluator(e.operator)(
            inColumnVal,
            e.values,
          );
        case 'unary_op':
...
const makePerformExistsSubquery =
  (
    ...
    performSubquery: (
      sourceRow: Record<string, RawScalarValue>,
...
const sortRows = (
  ...
  rows
    ...
    .sort(([lhs, lhsValueCache], [rhs, rhsValueCache]) => {
      return orderBy.elements.reduce(
        (accum, orderByElement, orderByElementIndex) => {
          if (accum !== 0) {
            return accum;
          }
          const leftVal: RawScalarValue =
            lhsValueCache[orderByElementIndex] !== undefined
              ...
              : (lhsValueCache[orderByElementIndex] = getOrderByElementValue(
                  orderByElement,
                  lhs,
                  orderBy.relations,
                ));
          const rightVal: RawScalarValue =
            rhsValueCache[orderByElementIndex] !== undefined
              ...
              : (rhsValueCache[orderByElementIndex] = getOrderByElementValue(
                  orderByElement,
                  rhs,
                  orderBy.relations,
                ));
          ...
          return orderByElement.order_direction === 'desc'
            ? -compared
            : compared;
        },
        0,
...
const paginateRows = (
  rows: Iterable<Record<string, RawScalarValue>>,
  offset: number | null,
  limit: number | null,
): Iterable<Record<string, RawScalarValue>> => {
  const skipped = offset !== null ? skipIterable(rows, offset) : rows;
  return limit !== null ? takeIterable(skipped, limit) : skipped;
};

const makeFindRelationship =
  (request: QueryRequest, targetName: TargetName) =>
...
const makeApplyRedaction = (
  ...
  const targetRedactionExpression = (request.redaction_expressions ?? []).find(
    (targetExps) => targetNameEquals(targetExps.target)(name),
  );

  // Redaction is not applied inside redaction expressions themselves!
  const getComparisonColumnValue = makeGetComparisonColumnValue(
    parentQueryRowChain,
    noRedaction,
  );
  const buildFilterPredicate = makeFilterPredicateBuilder(
    getComparisonColumnValue,
    performExistsSubquery,
  );
  ...
  const redactionPredicates =
    targetRedactionExpression !== undefined
      ? mapObjectValues(targetRedactionExpression.expressions, (redactionExp) =>
          buildFilterPredicate(redactionExp),
        )
      : {};
...
const createFilterExpressionForRelationshipJoin = (
  row: Record<string, RawScalarValue>,
  relationship: Relationship,
): Expression | null => {
  const columnMappings = Object.entries(relationship.column_mapping);
  const filterConditions: Expression[] = columnMappings
    .map(([outerColumnName, innerColumnName]): [RawScalarValue, string] => [
      row[outerColumnName],
      innerColumnName as string,
    ])
  ...
  if (
    ...
    filterConditions.length !== columnMappings.length
  ) {
    return null;
  } else {
...
const addRelationshipFilterToQuery = (
  row: Record<string, RawScalarValue>,
  relationship: Relationship,
  subquery: Query,
): Query | null => {
  const filterExpression = createFilterExpressionForRelationshipJoin(
    row,
    relationship,
  );
  ...
  if (filterExpression === null) {
    ...
    return {
      ...
      where: {
        type: 'and',
        expressions: [filterExpression, ...existingFilters],
      },
    };
  }
};

const makeGetComparisonColumnValue =
  (
    parentQueryRowChain: Record<string, RawScalarValue>[],
    applyRedaction: ApplyRedaction,
  ) =>
  (
    ...
    if (path.length === 0) {
      return applyRedaction(
        row,
        comparisonColumn.redaction_expression,
        coerceUndefinedToNull(row[getColumnSelector(comparisonColumn.name)]),
      );
    } else if (path.length === 1 && path[0] === '$') {
      const queryRow =
        parentQueryRowChain.length === 0 ? row : parentQueryRowChain[0];
      return coerceUndefinedToNull(
        queryRow[getColumnSelector(comparisonColumn.name)],
      );
    } else {
      throw new Error(
        `Unsupported path on ComparisonColumn: ${prettyPrintComparisonColumn(comparisonColumn)}`,
      );
    }
  };
...
const projectRow =
  ...
  (row: Record<string, RawScalarValue>): ProjectedRow => {
    ...
    for (const [fieldName, field] of Object.entries(fields)) {
      switch (field.type) {
        case 'column':
          projectedRow[fieldName] = applyRedaction(
            row,
            field.redaction_expression,
            coerceUndefinedToNull(row[field.column]),
          );
          break;

        case 'relationship':
          const relationship = findRelationship(field.relationship);
          const subquery = addRelationshipFilterToQuery(
            row,
            relationship,
            field.query,
          );
          ...
          switch (relationshipTarget.type) {
            case 'table':
              projectedRow[fieldName] = subquery
                ...
                : { aggregates: null, rows: null };
              break;
            default:
              throw new Error(
                `projectRow: relationships currently only work for tables - Target: ${JSON.stringify(relationshipTarget)}`,
              );
          }
          break;
...
const starCountAggregateFunction = (
  rows: Record<string, RawScalarValue>[],
): RawScalarValue => {
  return rows.length;
};

const columnCountAggregateFunction =
  (aggregate: ColumnCountAggregate, applyRedaction: ApplyRedaction) =>
  (rows: Record<string, RawScalarValue>[]): RawScalarValue => {
    const nonNullValues = rows
      .map((row) =>
        applyRedaction(
          row,
          aggregate.redaction_expression,
...
const isComparableArray = (
  values: RawScalarValue[],
...
const singleColumnAggregateFunction =
  (aggregate: SingleColumnAggregate, applyRedaction: ApplyRedaction) =>
  (rows: Record<string, RawScalarValue>[]): RawScalarValue => {
    const values = rows
      .map((row) =>
        applyRedaction(
      ...
      .filter((v): v is Exclude<RawScalarValue, null> => v !== null);
    if (values.length === 0) return null;

    if (!isComparableArray(values)) {
      throw new Error(
        `Found non-comparable scalar values when computing ${aggregate.function}`,
      );
    }
    ...
    if (!isNumberArray(values)) {
      throw new Error(
        `Found non-numeric scalar values when computing ${aggregate.function}`,
      );
    }
    switch (aggregate.function) {
      case 'avg':
      ...
      case 'stddev_pop':
        return math.std(values, 'uncorrected') as unknown as number;
      case 'stddev_samp':
        return math.std(values, 'unbiased') as unknown as number;
      case 'stddev':
        return math.std(values, 'unbiased') as unknown as number;
      case 'sum':
        return math.sum(values) as unknown as number;
      case 'var_pop':
        return math.variance(values, 'uncorrected') as unknown as number;
      case 'var_samp':
        return math.variance(values, 'unbiased') as unknown as number;
      case 'variance':
        return math.variance(values, 'unbiased') as unknown as number;
      default:
...
const calculateAggregates = (
  rows: Record<string, RawScalarValue>[],
  applyRedaction: ApplyRedaction,
  aggregateRequest: Record<string, Aggregate>,
): Record<string, RawScalarValue> => {
  return Object.fromEntries(
    Object.entries(aggregateRequest).map(([fieldName, aggregate]) => {
      const aggregateValue = getAggregateFunction(
        aggregate,
        applyRedaction,
      )(rows);
...
const makeForeachFilterExpression = (
  foreachFilterIds: Record<string, ScalarValue>,
): Expression => {
  const expressions: Expression[] = Object.entries(foreachFilterIds).map(
    ([columnName, columnScalarValue]) => ({
      type: 'binary_op',
      operator: 'equal',
      column: {
        name: columnName,
...
export const queryData = (
  getTable: (tableName: TableName) => Rows | undefined,
  ...
  const performQuery = (
    ...
    const performExistsSubquery = makePerformExistsSubquery(
      findRelationship,
      performSubquery,
    );
    const applyRedaction = makeApplyRedaction(
      queryRequest,
      targetName,
      parentQueryRowChain,
      performExistsSubquery,
    ...
    const getOrderByElementValue = makeGetOrderByElementValue(
      findRelationship,
      applyRedaction,
      performNewQuery,
    );

    const filteredRows = filterIterable(
      rows,
      buildFilterPredicate(query.where ?? null),
    );
    const sortedRows = query.order_by
      ? sortRows(
          Array.from(filteredRows),
          query.order_by,
          getOrderByElementValue,
        )
      : filteredRows;
    ...
    const largestLimit =
      aggregatesLimit !== null && rowLimit !== null
        ? Math.max(aggregatesLimit, rowLimit)
        : null;
    const largestPageOfRows = Array.from(
      paginateRows(sortedRows, query.offset ?? null, largestLimit),
    );

    // Limit the set of input rows to appropriate size for row results and aggregation results
    const paginatedRows =
      rowLimit != null
        ? largestPageOfRows.slice(0, rowLimit)
        : largestPageOfRows;
    const paginatedRowsForAggregation =
      aggregatesLimit != null
        ? largestPageOfRows.slice(0, aggregatesLimit)
        : largestPageOfRows;
  ...
  const performNewQuery = (
    targetName: TargetName,
    query: Query,
  ): QueryResponse => performQuery([], targetName, query, getTableRows);
  ...
  switch (rootTarget.type) {
    case 'function':
      const getRows = (
        targetName: TargetName,
      ): Record<string, RawScalarValue>[] | undefined => {
        switch (targetName.type) {
          case 'table':
            return getTable(targetName.table);
          case 'function':
            return respondToFunction(
              rootTarget.name,
              rootTarget.arguments ?? [],
              getTable,
            );
          case 'interpolated':
            throw new Error(
              "Can't perform a subquery using an interpolated query",
            );
          default:
            return unreachable(targetName['type']);
        }
      ...
      const result = performQuery(
        [],
        { type: 'function', function: rootTarget.name },
        queryRequest.query,
        getRows,
      );
      return result;
        ...
        return {
          rows: queryRequest.foreach.map((foreachFilterIds) => {
            ...
            const where: Expression = queryRequest.query.where
              ? {
                  type: 'and',
                  expressions: [foreachFilter, queryRequest.query.where],
                }
              : foreachFilter;
...
const expectedString = (x: string): never => {
  throw new Error(`Expected string value but got ${x}`);
};

const expectedNumber = (x: string): never => {
  throw new Error(`Expected number value but got ${x}`);
};

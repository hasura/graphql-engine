...
You can define a scheduled event via the `metadata_create_scheduled_event` metadata API:
...
You can define advanced options when defining a scheduled event via the `metadata_create_scheduled_event` metadata API:

...
You can add read replicas for a database using the [pg_add_source metadata API](/graphql/core/api-reference/metadata-api/source.mdx#pg-add-source)
...
:::info For existing v1.3 projects If you have configured your Postgres instances with replicas, the replica URLs can be added to Hasura using the following environment variable in your project ENV Vars tab:
...
You can add read replicas for a database using the using the `mssql_add_source metadata API <mssql_add_source>`

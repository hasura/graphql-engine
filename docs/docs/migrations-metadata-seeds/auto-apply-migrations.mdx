...
If you are using hasura.cli-migrations with docker container image as a CI/CD Pipeline, you might need to completely kill any hasura objects in your database in order to rebuild them.

Dependency issues may arise such as, *INSTEAD OF* triggers getting created while granting some partial *INSERT* permissions.

To resove such errors run the following commands:

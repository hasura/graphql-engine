...
func (b *BackendConfigConfig) CreateFiles() error {
	...
	err = os.WriteFile(filepath.Join(b.MetadataDir, b.Filename()), data, 0o644)
	if err != nil {
		return errors.E(op, err)
	}
...
func (b *BackendConfigConfig) Build() (map[string]any, error) {
	...
	return map[string]any{b.Key(): obj}, nil
}
...
func (b *BackendConfigConfig) Export(metadata map[string]yaml.Node) (map[string][]byte, error) {
	...
	m, err := metadataobject.DefaultExport(
		b,
		metadata,
		b.error,
...
func (b *BackendConfigConfig) WriteDiff(opts metadataobject.WriteDiffOpts) error {
	...
	err := metadataobject.DefaultWriteDiff(
		metadataobject.DefaultWriteDiffOpts{From: b, WriteDiffOpts: opts},
	)
	if err != nil {
...
func (b *BackendConfigConfig) error(
	err error,

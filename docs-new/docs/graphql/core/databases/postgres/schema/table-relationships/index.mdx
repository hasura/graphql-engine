...
|---------------|--------------------------------|--------------------------------------------------------------------------------------|
|`one-to-one`   | `authors` and `passport_infos` | an `author` can have one `passport_info` \| a `passport_info` can have one `owner`   |
|`one-to-many`  | `authors` and `articles`       | an `author` can have many `articles` \| an `article` can have one `author`           |	
|`many-to-many` | `articles` and `tags`          | an `article` can have many `tags` \| a `tag` can have many `articles`                |

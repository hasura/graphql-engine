...
PostgreSQL `@?` operator with a [SQL/JSON path](https://www.postgresql.org/docs/current/functions-json.html#FUNCTIONS-SQLJSON-PATH)
expression:

<GraphiQLIDE
  query={`query get_large_orders {
  orders(
    where: {
      items: {
        _jsonb_path_exists: "$[*] ? (@.quantity > 5)"
      }
    }
  ) {
    id
...
[SQL/JSON path](https://www.postgresql.org/docs/current/functions-json.html#FUNCTIONS-SQLJSON-PATH) predicate expression:

<GraphiQLIDE
  query={`query get_affordable_products {
  products(
    where: {
      metadata: {
        _jsonb_path_match: "$.price < 50"
      }
    }
  ) {
    id
    ...
    "products": [
      {
        "id": 3,
        "name": "Widget",
        "metadata": { "price": 29.99, "stock": 100 }
      }
    ]
  }
}`}

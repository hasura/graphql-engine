...
func (s *Stub) First() (version uint64, err error) {
	...
	if v, ok := s.Migrations.First(); !ok {
		return 0, errors.E(
			op,
			&os.PathError{Op: "first", Path: s.Url, Err: os.ErrNotExist},
		) // TODO: s.Url can be empty when called with WithInstance
	} else {
		return v, nil
	}
}
...
func (s *Stub) ReadUp(
	version uint64,
) (r io.ReadCloser, identifier string, fileName string, err error) {
	var op errors.Op = "stub.Stub.ReadUp"

	if m, ok := s.Migrations.Up(version); ok {
		return io.NopCloser(
				bytes.NewBufferString(m.Identifier),
			), fmt.Sprintf(
				"%v.up.sql.stub",
				version,
			), fmt.Sprintf(
	...
	return nil, "", "", errors.E(
		op,
		&os.PathError{
			Op:   fmt.Sprintf("read up sql version %v", version),
			Path: s.Url,
			Err:  os.ErrNotExist,
		},
	)
...
func (s *Stub) ReadDown(
	...
	return nil, "", "", errors.E(
		op,
		&os.PathError{
			Op:   fmt.Sprintf("read down sql version %v", version),
			Path: s.Url,
			Err:  os.ErrNotExist,
		},
	)
...
func (s *Stub) ReadMetaUp(
	...
	return nil, "", "", errors.E(
		op,
		&os.PathError{
			Op:   fmt.Sprintf("read up yaml version %v", version),
			Path: s.Url,
			Err:  os.ErrNotExist,
		},
	)
...
func (s *Stub) ReadMetaDown(
	...
	return nil, "", "", errors.E(
		op,
		&os.PathError{
			Op:   fmt.Sprintf("read down yaml version %v", version),
			Path: s.Url,
			Err:  os.ErrNotExist,
		},
	)

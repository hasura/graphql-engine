...
You can run multiple updates in sequence, each with its own `where` and `update` clauses.
...
The result is an array, and each distinct entry in `updates` will result in the respective array result. If any of
the updates error for whatever reason, all the others are rolled back as updates are executed inside a
transaction.

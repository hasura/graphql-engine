...
database _(which is by default the same database from which data is
served over the GraphQL API if it is Postgres)_. When initialized, the
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>perm_def</td>
      <td>
      Captures information about how the relationship is defined. For example:
      </td>
    </tr>
    <tr>
      <td>comment</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      Whenever a request is made with the above role for the above table GraphQL engine will first validate the requested columns with the columns which the user has access to using the <code>columns</code> key.
      Once the request is validated the appropriate results are returned after applying the filter defined in the <code>filter</code> key.
      </td>
    </tr>
    <tr>
      <td>comment</td>
...
Whenever the schema of the catalogue is modified _(typically to support
new features)_ a new version of the catalogue is generated.

...
mechanism into two different threads is that, in most of the cases the
set of the scheduled events generated by the graphql-engine and the set of
the scheduled events that are supposed to be delivered will never be the same
because the scheduled events generated by the graphql-engine will in most
of the cases be a scheduled event with a future timestamp (greater
than current_timestamp) and the scheduled events that are supposed
to be delivered will be scheduled events with a timestamp
lesser than the current timestamp.

Now, comparing scheduled triggers to event triggers, the delivery mechanism
is similar because in both the cases, a thread monitors if there are any new
events to be delivered and if there are then it makes an HTTP POST call to the
webhook server.

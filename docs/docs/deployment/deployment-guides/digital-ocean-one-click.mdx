...
### Step 5: Persist the Caddy certs volume

This step is important to avoid caddy getting rate-limited if it has to refetch your certs too often due to docker restarts.
...
docker volume create --name=caddy_certs
```

configure the volume as external in `docker-compose.yml`

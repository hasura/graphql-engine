...
const isErrorWithStatusCode = (
  ...
  if (error instanceof Error) {
    return error.name === 'ErrorWithStatusCode';
  }
...
(() => {
  ...
  server.addHook('onRequest', async (request, reply) => {
    requestCounter.inc({ route: request.routeOptions.url });
  });
})();
...
server.addHook('onRequest', async (request, reply) => {
  if (request.routeOptions.url === '/schema') request.raw.method = 'POST';
});
...
if (DATASETS) {
  server.get<{
    Params: { template_name: DatasetTemplateName };
    Reply: DatasetGetTemplateResponse;
  }>('/datasets/templates/:template_name', async (request, _response) => {
    server.log.info(
      { headers: request.headers, query: request.body },
      'datasets.templates.get',
    );
  ...
  server.post<{
    Params: { clone_name: string };
    Body: DatasetCreateCloneRequest;
    Reply: DatasetCreateCloneResponse;
  }>('/datasets/clones/:clone_name', async (request, _response) => {
    server.log.info(
      { headers: request.headers, query: request.body },
      'datasets.clones.post',
    );

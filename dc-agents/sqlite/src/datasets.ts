...
): Promise<DatasetGetTemplateResponse> {
  const templatePaths = mkTemplatePaths(template_name);
  return {
    exists:
      (await fileIsReadable(templatePaths.dbFileTemplatePath)) ||
      (await fileIsReadable(templatePaths.sqlFileTemplatePath)),
  };
}
...
): Promise<DatasetCreateCloneResponse> {
  ...
  if (await fileIsReadable(templatePaths.dbFileTemplatePath)) {
    try {
      await withConnection(
        {
          db: templatePaths.dbFileTemplatePath,
    ...
    const sql = await promises.readFile(templatePaths.sqlFileTemplatePath, {
      encoding: 'utf-8',
    });
    await withConnection(
      { db: toPath, explicit_main_schema: false, tables: [], meta: false },
      createDbMode,
      logger,
      async (db) => {
...
export async function deleteDataset(
  clone_name: string,
): Promise<DatasetDeleteCloneResponse> {
  if (DATASET_DELETE) {

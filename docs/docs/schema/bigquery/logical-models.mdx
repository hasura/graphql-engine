...
[Native Queries](/schema/bigquery/native-queries.mdx) feature to automatically create a GraphQL API for a native
database query.

You can find examples of how to use Logical Models in the [Native Queries](/schema/bigquery/native-queries.mdx)
...
The filter is the same boolean expression syntax as [query filters](/queries/bigquery/filters/index.mdx). To allow all

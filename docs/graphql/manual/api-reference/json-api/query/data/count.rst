...
Description
^^^^^^^^^^^
``count`` returns a count of the rows that satisfy the ``"where"`` condition
from the specified table. 

If ``"distinct"`` is specified, all duplicate rows are not counted (one row is
counted from each group of duplicates). 

You must have ``select`` permission on the table and on all the columns used in
``"where"`` and ``"distinct"``. 

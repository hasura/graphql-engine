...
You can define a cron trigger via the `metadata_create_cron_trigger` metadata API:
...
<TabItem value="api" label="API">
You can define advanced options for a cron trigger when defining it via the `metadata_create_cron_trigger` metadata API:

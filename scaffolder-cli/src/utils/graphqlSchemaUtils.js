const { isWrappingType, isListType, isNonNullType } = require('graphql');

const getMutationType = (schema) => {
  return schema._mutationType;
}

const getUnderlyingType = (_type) => {
...
const getTypeFields = _type => {
  return _type._fields || [];
};

const getFieldArgs = (field) => {
  return field.args || [];
}

const getHasuraMutationMetadata = (field) => {
  if (
    field.args.length === 2 &&

...
This requirement does not apply when `HASURA_GRAPHQL_DISABLE_ADMIN_SECRET=true`.

:::

### Disable Admin Secret

Authentication via the `x-hasura-admin-secret` request header is disabled.

|                     |                                       |
| ------------------- | ------------------------------------- |
| **Flag**            | `--disable-admin-secret`              |
| **Env var**         | `HASURA_GRAPHQL_DISABLE_ADMIN_SECRET` |
| **Accepted values** | Boolean                               |
| **Options**         | `true` or `false`                     |
| **Default**         | `false`                               |
| **Supported in**    | CE and Enterprise Edition             |

When enabled in CE, you must configure either `HASURA_GRAPHQL_AUTH_HOOK` or `HASURA_GRAPHQL_JWT_SECRET`. In self-hosted
Enterprise Edition, you must configure at least one of `HASURA_GRAPHQL_AUTH_HOOK`, `HASURA_GRAPHQL_JWT_SECRET`,
`HASURA_GRAPHQL_JWT_SECRETS`, or `HASURA_GRAPHQL_SSO_PROVIDERS`.

:::warning CE console and metadata APIs

The CE Console cannot authenticate when admin-secret authentication is disabled. For CE deployments, set
`HASURA_GRAPHQL_ENABLE_CONSOLE=false` and `HASURA_GRAPHQL_ENABLED_APIS=graphql` unless you intentionally expose another
API configuration.
...
### Enable Naming Convention Sep 2023 feature flag

By default, even when `graphql-default` is selected, the following names keep using `hasura-default`-style (snake_case)
casing, to preserve backward compatibility with schemas generated before September 2023. Enable this flag to opt in to
the full `graphql-default`.

|                     |                                        |
| ------------------- | -------------------------------------- |
| **Env var**         | `HASURA_FF_NAMING_CONVENTION_SEP_2023` |
| **Accepted values** | Boolean                                |
| **Options**         | `true` or `false`                      |
| **Default**         | `false`                                |
| **Supported in**    | CE, Enterprise Edition, Cloud          |
...
### Redact Event Trigger Logs

When enabled, redacts the request body, session variables and webhook response body from the **Event Trigger**
(`event-trigger`) logs. The webhook URL and sensitive/from-env headers are always redacted regardless of this flag.
Redacted fields are emitted as `null`. Enable this to avoid leaking sensitive payload data into `stdout` logs when a
webhook delivery fails (for example, when re-enabling event-trigger error logging in `stdout`).

|                     |                                            |
| ------------------- | ------------------------------------------ |
| **Flag**            | `--redact-event-trigger-logs`              |
| **Env var**         | `HASURA_GRAPHQL_REDACT_EVENT_TRIGGER_LOGS` |
| **Accepted values** | Boolean                                    |
| **Default**         | `false`                                    |
| **Supported in**    | CE, Enterprise Edition                     |

### Redact Scheduled Trigger Logs

When enabled, redacts the request body, session variables and webhook response body from the **Scheduled/Cron Trigger**
(`scheduled-trigger`) logs (covers cron and one-off scheduled events). Redacted fields are emitted as `null`.

|                     |                                                |
| ------------------- | ---------------------------------------------- |
| **Flag**            | `--redact-scheduled-trigger-logs`              |
| **Env var**         | `HASURA_GRAPHQL_REDACT_SCHEDULED_TRIGGER_LOGS` |
| **Accepted values** | Boolean                                        |
| **Default**         | `false`                                        |
| **Supported in**    | CE, Enterprise Edition                         |

### Redact Action Handler Logs

When enabled, redacts the request body (of both the original and the transformed request) from the **Action handler**
(`action-handler-log`) logs. The request size, response size and action metadata are always retained. Redacted fields
are emitted as `null`.

|                     |                                             |
| ------------------- | ------------------------------------------- |
| **Flag**            | `--redact-action-handler-logs`              |
| **Env var**         | `HASURA_GRAPHQL_REDACT_ACTION_HANDLER_LOGS` |
| **Accepted values** | Boolean                                     |
| **Default**         | `false`                                     |
| **Supported in**    | CE, Enterprise Edition                      |
...
Enable the Relay API endpoint (`/v1/relay` and `/v1beta1/relay`). When disabled, requests to the relay endpoint return
an error with instructions to enable it. Disabling this reduces memory usage for deployments that do not use Relay.
...
Preserves HTTP 401 (Unauthorized) status codes from webhook auth responses and JWT auth failures. By default, all
GraphQL errors are returned with HTTP status code 200, even authentication errors. When enabled, 401 errors will be
...
Maximum number of connections in the caching Redis connection pool. For high-traffic production environments, you may
want to increase this value. For development or low-traffic environments, you can decrease it to conserve resources.
...
Maximum number of connections in the rate-limiting Redis connection pool. For high-traffic production environments with
heavy rate-limiting usage, you may want to increase this value. For development or low-traffic environments, you can
decrease it to conserve resources.
...
Sets the maximum number of subscription-result and query-result messages buffered per WebSocket connection. The
primary purpose is to bound memory usage when a subscriber is slow to consume messages — without a limit, a stalled
or slow client can cause unbounded memory growth on the server as results accumulate.

Unlike a simple bounded buffer, nothing already queued is ever silently dropped to make room for newer messages. When
the queue is at capacity, a new result for an operation isn't enqueued at all: that operation is stopped instead, and
the client receives an explicit `error` followed by a `complete` telling it to retry (a one-shot query or mutation)
or resubscribe (a live query or streaming subscription). This applies uniformly to every operation kind, including
live queries: it's only correct to skip a live query's update when a _newer_ one supersedes it. If the connection
stayed overloaded indefinitely, a subscriber could otherwise be stuck on stale data forever with no indication
anything was wrong — resubscribing re-fetches the current state immediately, so nothing is permanently missed.

Control/signal messages (completions, errors, connection acks, pongs) are never subject to this limit — they always
succeed, so the queue's true depth can exceed this setting slightly under sustained overcapacity, bounded by however
many operations are concurrently being stopped this way, not unbounded.

The `hasura_websocket_messages_dropped_total` Prometheus metric counts how many times a data message couldn't be
enqueued this way; pairing it with `hasura_websocket_messages_queued_total` gives the drop rate as a fraction of
total sends.
...
**Streaming subscriptions:** Streaming queries present results as an ordered sequence of elements and rely on
cursor-based pagination to make progress, so the error a client receives on overcapacity also includes, in its
`extensions.resume_cursor` field, the cursor value to resubscribe with in order to resume from exactly where it left
off — no data is skipped or duplicated. Streaming subscriptions require the `streaming_subscriptions` experimental
feature flag:

    {
      "errors": [
        {
          "message": "This operation's result could not be delivered because the connection's send queue was at capacity, so it has been stopped. Please retry, using this cursor as the initial_value to resume from exactly where results were lost: {\"TrackId\":\"1328\"}.",
          "extensions": {
            "code": "unexpected",
            "path": "$",
            "resume_cursor": {
...
### Masked request variables in query-log

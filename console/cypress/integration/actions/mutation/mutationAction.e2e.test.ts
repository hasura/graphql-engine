...
import { loginActionMustExist } from './utils/testState/loginActionMustExist';
import { mustBeOnTheActionsPage } from './utils/testState/mustBeOnTheActionsPage';
import { loginActionMustNotExist } from './utils/testState/loginActionMustNotExist';
import { hakunaMatataPermissionMustNotExist } from './utils/testState/hakunaMatataPermissionMustNotExist';
...
function runTests() {
  describe('Mutation Actions', () => {
    ...
    // FYI: This is the same flow described in the Hasura's official documentation, under the
    // "Creating Actions" section https://hasura.io/docs/latest/graphql/core/actions/create/
    describe('When the user creates a new Action, everything should work', () => {
      before(() => {
        loginActionMustNotExist();
        mustBeOnTheActionsPage();
      });
    ...
    describe('When the user add a permission, everything should work', () => {
      before(() => {
        loginActionMustExist();
        hakunaMatataPermissionMustNotExist();
        mustBeOnTheActionsPage();
      });
    ...
    describe('When the user deletes a Action, everything should work', () => {
      before(() => {
        loginActionMustExist();
        mustBeOnTheActionsPage();
      });
...
// TODO: move to https://github.com/cypress-io/cypress-skip-test
if (testMode !== 'cli') {
  runTests();
}

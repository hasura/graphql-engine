...
const TodoPublicList = (props) => {
  ...
  const [todosTry, setTodosTry] = useState([]);

  // client = props.client;
  let oldestTodoId = props.latestTodo ? (props.latestTodo.id + 1) : 0;
  let newestTodoId = props.latestTodo ? props.latestTodo.id : 0;
  ...
  useEffect(() => {
    if (props.latestTodo && props.latestTodo.id > newestTodoId) {
      ...
      setState({...state, newTodosCount: state.newTodosCount + 1});
    }
  }, [props.latestTodo]);
    ...
    const { loading, error, data} = await client.query({query: GET_OLD_PUBLIC_TODOS, 
      variables: {oldestTodoId: oldestTodoId}
          ...
          if (data.todos.length) {
            ...
            setState({...state ,todos: [...state.todos, ...data.todos]});
            setTodosTry(prevTodosTry => [...prevTodosTry, ...data.todos])
            console.log("state", state)
            console.log("data", todosTry)
          } else {
...
const { loading, error, data, client} = useQuery(GET_NEW_PUBLIC_TODOS, {
  variables: {latestVisibleId: state.todos.length ? state.todos[0].id : null} 
});   

      if(data) {  
        newestTodoId = data.todos[0].id;
        setState({...state, todos: [...data.todos, ...state.todos], newTodosCount: 0});
      }
      if(error){
        console.error(error);
        setState({...state, error: true});

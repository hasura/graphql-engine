...
const getAstEntityDescription = (def) =>
  def.description ? def.description.value.trim() : null;
...
const getArgumentsSdl = (args) => {
  ...
  const argsSdl = args.map(
    (a) => `    ${getEntityDescriptionSdl(a)}${a.name}: ${a.type}`
  );
...
const getObjectTypeSdl = (type) => `${getEntityDescriptionSdl(type)}type ${
  type.name
} {
${getFieldsSdl(type.fields)}
}\n\n`;

const getInputTypeSdl = (type) => `${getEntityDescriptionSdl(type)}input ${
  type.name
} {
${getFieldsSdl(type.fields)}
}\n\n`;
...
const getScalarTypeSdl = (type) =>
  `${getEntityDescriptionSdl(type)}scalar ${type.name}\n\n`;

const getEnumTypeSdl = (type) => {
  const enumValuesSdl = type.values.map(
    (v) => `  ${getEntityDescriptionSdl(v)}${v.value}`
  );
...
export const getAllTypesFromSdl = (sdl) => {
  ...
  const types = ast.definitions.map((d) => getTypeFromAstDef(d));
...
export const toggleCacheDirective = (operationString) => {
  ...
  const shouldAddCacheDirective = !operationAst.definitions.some((def) =>
    def.directives.some((dir) => dir.name.value === 'cached')
  );

...
If the `Set-Cookie` HTTP headers are set by the auth webhook, they are forwarded by the GraphQL Engine as response
...
There is no default timeout on authorized websocket connection. You can optionally add one; to do so, you need to return
either:
...
For websockets, if the `Cache-Control` or `Expires` fields are present in the response
then a new request to the auth-webhook is made after the time specified in those
fields and a new websocket connection is established.

...
- **Headers**: configure the headers to be sent to your custom GraphQL server:
  ...
  - Optionally use a separate list of introspection headers. If enabled with an empty list, introspection sends no
    configured headers. If disabled, introspection uses the additional headers above for backward compatibility. Runtime
    operations always use the additional headers.

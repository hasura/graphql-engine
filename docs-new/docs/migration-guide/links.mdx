...
While the top level can be added as `id` attribute in metadata. The sub-section (h2's, h3's and stuff) can have id's with speacial mdx syntax `{#id-for-sub-section}`

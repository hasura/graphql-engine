...
func (r *relationshipSuggestion) Track(tableName, schemaName string) hasuraDBQuery {
	query := hasuraDBQuery{}
	switch r.RelType {
	case "object":
		...
		if len(r.ColumnMapping) == 1 {
			...
			object.Using = objRelUsing{
				ForeignKeyConstraintOn: columns[0],
			}
		} else {
	...
	case "array":
		...
		if len(r.ColumnMapping) == 1 {
			// Use foreign_key_constraint_on
			columns := getColumnMappingKeySlice(r.ColumnMapping)
			array.Using = arrayRelUsing{
				ForeignKeyConstraintOn: &arrayForeignKeyConstraintOn{
					Table: tableArg{
						Name:   r.RefTableName,
						Schema: r.RefTableSchema,
					},
...
func (r *relationshipSuggestion) UnTrack(tableName, schemaName string) hasuraDBQuery {
...
func (r *relationshipSuggestion) generateName(names []string) string {
	var relName string

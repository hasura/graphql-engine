...
import           Hasura.GraphQL.RemoteServer       (customizeIntrospectionResult, getCustomizers,
                                                    identityCustomizer, introspectionResultToJSON,
                                                    parseIntrospectionResult)
import           Hasura.Prelude
import           Hasura.RQL.DDL.Metadata.Generator ()
...
spec = do
    describe "IntrospectionResult" $ do
        prop "JSON roundtrip" $ forAll (scale (`div` 4) arbitrary) $ -- Use scale to ensure tests run in a reasonable time
    ...
    describe "getCustomizers" $ do
        prop "inverse" $ forAll gen $ \(introspectionResult, typesAndFields, customization) ->
            let (customizer, decustomizer) = getCustomizers introspectionResult (Just customization)
                RemoteSchemaCustomizer _ customizeTypeName customizeFieldName = customizer
                RemoteSchemaCustomizer _ decustomizeTypeName decustomizeFieldName = decustomizer

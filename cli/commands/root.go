...
var rootCmd = &cobra.Command{
	...
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
		if cmd.Use != updateCLICmdUse {
			if update.ShouldRunCheck(ec.LastUpdateCheckFile) &&
				...
				if err != nil && u.EC.Version.GetCLIVersion() != version.DevVersion {
					ec.Logger.WithError(err).
						Warn("auto-update failed, run 'hasura update-cli' to update manually")
				}
			}
		}
	},
...
func init() {
	...
	f.StringVar(
		&ec.ExecutionDirectory,
		"project",
		"",
		"directory where commands are executed (default: current dir)",
	)
	f.BoolVar(
		&ec.SkipUpdateCheck,
		"skip-update-check",
		false,
		"skip automatic update check on command execution",
	)
...
func NewDefaultHasuraCommand() *cobra.Command {
	return NewDefaultHasuraCommandWithArgs(
		NewDefaultPluginHandler(validPluginFilenamePrefixes),
		os.Args,
		os.Stdin,
		os.Stdout,
		os.Stderr,
...
func NewDefaultHasuraCommandWithArgs(
	...
	if len(args) > 1 {
		...
		if _, _, err := cmd.Find(cmdPathPieces); err != nil {
			err := HandlePluginCommand(pluginHandler, cmdPathPieces)
			if err != nil {

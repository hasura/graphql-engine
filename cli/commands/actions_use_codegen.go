...
func newActionsUseCodegenCmd(ec *cli.ExecutionContext) *cobra.Command {
	...
	actionsUseCodegenCmd := &cobra.Command{
		...
		Long:  "Running `hasura actions use-codegen` generates code for Hasura Actions using the codegen framework of your choice. While not required, you can pass the `--framework` flag to select a framework. If you do not pass the `--framework` flag, you will be prompted to select a framework from a list of available options.",

...
:::note Risk of using session variables
...
in) users. See [GraphQL Engine server config reference](/graphql/core/deployment/graphql-engine-flags/reference.mdx#server-flag-reference) for more details on setting this
flag/env var.

...
func getCliExtFileContent(ec *cli.ExecutionContext) ([]byte, error) {
	...
	if ec.CliExtSourceBinPath != "" {
		...
		b, err := os.ReadFile(ec.CliExtSourceBinPath)
		if err != nil {
	...
	cliExtFileContent, err := cliExtFS.ReadFile(
		filepath.ToSlash(filepath.Join("static-bin", runtime.GOOS, runtime.GOARCH, cliExtBinName)),
	)
	if err == nil {
	...
	CDNpath := fmt.Sprintf(
		"https://graphql-engine-cdn.hasura.io/cli-ext/releases/versioned/%s/cli-ext-%s-%s",
		cliExtVersion,
		runtime.GOOS,
		runtime.GOARCH,
	)
	...
	if resp.StatusCode != http.StatusOK {
		return nil, errors.E(
			op,
			fmt.Errorf(
				"error downloading cli-ext from CDN %s with status code: %d",
				CDNpath,
				resp.StatusCode,
			),
		)
...
func Setup(ec *cli.ExecutionContext) error {
	...
	if err != nil {
		return errors.E(
			op,
			fmt.Errorf("error creating base directory while setting up cli-ext: %w", err),
		)
	}

	cliExtDirPath, err := os.MkdirTemp(parentDirPath, "cli-ext-*")
	if err != nil {
		return errors.E(
			op,
			fmt.Errorf("error creating directory while setting up cli-ext: %w", err),
		)
	}
	...
	err = os.WriteFile(cliExtBinPath, cliExtFileContent, 0o755)
	if err != nil {

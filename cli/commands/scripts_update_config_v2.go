...
func newScriptsUpdateConfigV2Cmd(ec *cli.ExecutionContext) *cobra.Command { //nolint:funlen
	v := viper.New()
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if ec.Config.Version != cli.V1 {
				return errors.E(
					op,
					stderrors.New(
						"this script can be executed only when the current config version is 1",
					),
				)
			}
			...
			err = util.CopyDir(
				ec.MigrationDir,
				filepath.Join(ec.ExecutionDirectory, "migrations_backup"),
			)
			if err != nil {
				return errors.E(
					op,
					fmt.Errorf("error in copying migrations to migrations_backup: %w", err),
				)
			}
			...
			for _, version := range fileCfg.Migrations.Index {
				...
				if err != nil {
					return errors.E(
						op,
						fmt.Errorf("error in reading %s file: %w", upMetaMigration.Raw, err),
					)
				}
					...
					if !ok {
						...
						if upMetaMigration.IsDir {
							...
							fileName := fmt.Sprintf(
								"%d_%s.up.sql",
								version,
								upMetaMigration.Identifier,
							)
							filePath = filepath.Join(ec.MigrationDir, fileName)
						}

						err = os.WriteFile(filePath, sqlUp.Bytes(), os.ModePerm)
						if err != nil {
							return errors.E(
								op,
								fmt.Errorf("unable to create up migration: %w", err),
							)
						}
						...
						upByt, err := os.ReadFile(filePath)
						if err != nil {
						...
						err = os.WriteFile(filePath, upByt, os.ModePerm)
						if err != nil {
			...
			for _, version := range fileCfg.Migrations.Index {
				...
				if err != nil {
					return errors.E(
						op,
						fmt.Errorf("error in reading %s file: %w", downMetaMigration.Raw, err),
					)
				}
				...
				if err != nil {
					return errors.E(
						op,
						fmt.Errorf("unable to unmarshal %s: %w", downMetaMigration.Raw, err),
					)
				}
					...
					if !ok {
						...
						if downMetaMigration.IsDir {
							...
							fileName := fmt.Sprintf(
								"%d_%s.down.sql",
								version,
								downMetaMigration.Identifier,
							)
							filePath = filepath.Join(ec.MigrationDir, fileName)
						}

						err = os.WriteFile(filePath, sqlDown.Bytes(), os.ModePerm)
						if err != nil {
							return errors.E(
								op,
								fmt.Errorf("unable to create up migration: %w", err),
						...
						downByt, err := os.ReadFile(filePath)
						if err != nil {
						...
						err = os.WriteFile(filePath, downByt, os.ModePerm)
						if err != nil {
			...
			for version := range fileCfg.Migrations.Migrations {
				...
				if !directions[source.Up] && !directions[source.MetaUp] &&
					...
					files, err := filepath.Glob(
						filepath.Join(ec.MigrationDir, fmt.Sprintf("%d_*", version)),
					)
					if err != nil {
						return errors.E(
							op,
							fmt.Errorf("unable to filter files for %d: %w", version, err),
						)
					}

...
This recipe shows how to use Hasura's Event Triggers to automatically send a notification to a status of their order changes.
We'll build a webhook that parses Event Trigger data and sends a notification. [Check it out](/event-triggers/recipes/order-status-notification.mdx)!

...
You can add a remote schema by using the [add_remote_schema](/api-reference/metadata-api/remote-schemas.mdx#metadata-add-remote-schema) metadata API:
...
If you are running Hasura using Docker, ensure that the Hasura Docker container can reach the server endpoint. See [this page](/guides/docker-networking.mdx) for Docker networking.
...
Make a request to the [reload_remote_schema](/api-reference/metadata-api/remote-schemas.mdx#metadata-reload-remote-schema) metadata API.

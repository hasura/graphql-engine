...
import           Hasura.Incremental     (Cacheable, unchanged)
...
instance i `SatisfiesForAllBackends` Cacheable => Cacheable (AnyBackend i) where
  unchanged x e1 e2 =
    -- generates the following case expression for all backends:
    --   (FooValue a, FooValue b) -> unchanged x a b
    --   ...
    --   _ -> False
    $(backendCase [| (e1, e2) |]
       -- the pattern for a given backend

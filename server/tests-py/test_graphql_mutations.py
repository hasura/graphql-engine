...
class TestGraphQLInsert(DefaultTestMutations):
    ...
    def test_inserts_various_postgres_types(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_various_postgres_types.yaml", transport)
    ...
    def test_insert_into_array_col_with_array_input(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_into_array_col_with_array_input.yaml", transport)

    def test_insert_using_variable(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_jsonb_variable.yaml", transport)

    def test_insert_using_array_variable(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_jsonb_variable_array.yaml", transport)

    def test_insert_person(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_jsonb.yaml", transport)

    def test_insert_person_array(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_jsonb_array.yaml", transport)

    def test_insert_null_col_value(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/order_col_shipped_null.yaml", transport)
...
class TestGraphqlInsertOnConflict(DefaultTestMutations):
    ...
    def test_on_conflict_update(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/article_on_conflict_update.yaml", transport)

    def test_on_conflict_ignore(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/article_on_conflict_ignore_constraint.yaml", transport)

    def test_on_conflict_update_empty_cols(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/article_on_conflict_empty_update_columns.yaml", transport)

    def test_err_missing_article_constraint(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/article_on_conflict_error_missing_article_constraint.yaml", transport)

    def test_err_unexpected_action(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/article_unexpected_on_conflict_action.yaml", transport)
...
class TestGraphqlInsertPermission(DefaultTestMutations):
    ...
    def test_user_role_on_conflict_update(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/article_on_conflict_user_role.yaml", transport)

    def test_user_role_on_conflict_constraint_on_error(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/article_on_conflict_constraint_on_user_role_error.yaml", transport)

    def test_user_role_on_conflict_ignore(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_on_conflict_ignore_user_role.yaml", transport)

    def test_user_err_missing_article_constraint(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/user_article_on_conflict_error_missing_article_constraint.yaml", transport)

    def test_user_err_unexpected_action(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/user_article_error_unexpected_on_conflict_action.yaml", transport)

    def test_user_err_unexpected_constraint(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/user_article_unexpected_on_conflict_constraint_error.yaml", transport)

    def test_role_has_no_permissions_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/address_permission_error.yaml", transport)

    def test_author_user_role_insert_check_perm_success(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_user_role_insert_check_perm_success.yaml", transport)

    def test_user_role_insert_check_is_registered_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_user_role_insert_check_is_registered_fail.yaml", transport)

    def test_user_role_insert_check_user_id_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_user_role_insert_check_user_id_fail.yaml", transport)

    def test_student_role_insert_check_bio_success(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_student_role_insert_check_bio_success.yaml", transport)

    def test_student_role_insert_check_bio_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_student_role_insert_check_bio_fail.yaml", transport)

    def test_company_user_role_insert(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/company_user_role.yaml", transport)

    def test_company_user_role_insert_on_conflict(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/company_user_role_on_conflict.yaml", transport)

    def test_resident_user_role_insert(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/resident_user.yaml", transport)

    def test_resident_infant_role_insert(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/resident_infant.yaml", transport)

    def test_resident_infant_role_insert_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/resident_infant_fail.yaml", transport)
    ...
    def test_arr_sess_var_insert_article_as_editor_allowed_user_id(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_article_arr_sess_var_editor_allowed_user_id.yaml", transport)

    def test_arr_sess_var_insert_article_as_editor_err_not_allowed_user_id(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_article_arr_sess_var_editors_err_not_allowed_user_id.yaml", transport)

    def test_seller_insert_computer_json_has_keys_all(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/seller_insert_computer_has_keys_all_pass.yaml", transport)

    def test_seller_insert_computer_json_has_keys_all_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/seller_insert_computer_has_keys_all_fail.yaml", transport)

    def test_developer_insert_computer_json_has_keys_any(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/developer_insert_has_keys_any_pass.yaml", transport)
...
class TestGraphqlInsertConstraints(DefaultTestQueries):
    ...
    def test_address_not_null_constraint_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/address_not_null_constraint_error.yaml", transport)

    def test_insert_unique_constraint_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_unique_constraint_error.yaml", transport)
...
class TestGraphqlInsertGeoJson(DefaultTestMutations):
    ...
    def test_insert_point_landmark(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_landmark.yaml", transport)

    def test_insert_3d_point_drone_loc(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_drone_3d_location.yaml", transport)

    def test_insert_landmark_single_position_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_landmark_single_position_err.yaml", transport)

    def test_insert_line_string_road(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_road.yaml", transport)

    def test_insert_road_single_point_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_road_single_point_err.yaml", transport)

    def test_insert_multi_point_service_locations(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_service_locations.yaml", transport)
    def test_insert_multi_line_string_route(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_route.yaml", transport)

    def test_insert_polygon(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_area.yaml", transport)

    def test_insert_linear_ring_less_than_4_points_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_area_less_than_4_points_err.yaml", transport)

    def test_insert_linear_ring_last_point_not_equal_to_first_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_linear_ring_last_point_not_equal_to_first_err.yaml", transport)

    def test_insert_multi_polygon_compounds(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_compounds.yaml", transport)

    def test_insert_geometry_collection(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_geometry_collection.yaml", transport)

    def test_insert_unexpected_geometry_type_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_geometry_unexpected_type_err.yaml", transport)
...
class TestGraphqlNestedInserts(DefaultTestMutations):
    ...
    def test_author_with_articles(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_with_articles.yaml", transport)

    def test_author_with_articles_empty(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_with_articles_empty.yaml", transport)

    def test_author_with_articles_null(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_with_articles_null.yaml", transport)

    def test_author_with_articles_author_id_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_with_articles_author_id_fail.yaml", transport)

    def test_articles_with_author(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/articles_with_author.yaml", transport)

    def test_articles_with_author_author_id_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/articles_with_author_author_id_fail.yaml", transport)

    def test_author_upsert_articles_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_upsert_articles_fail.yaml", transport)

    def test_articles_author_upsert_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/articles_author_upsert_fail.yaml", transport)

    def test_articles_with_author_returning(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/articles_with_author_returning.yaml", transport)
...
class TestGraphqlInsertViews(DefaultTestMutations):
    ...
    def test_insert_view_author_simple(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_view_author_simple.yaml", transport)

    def test_insert_view_author_complex_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/insert_view_author_complex_fail.yaml", transport)

    def test_nested_insert_article_author_simple_view(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/nested_insert_article_author_simple_view.yaml", transport)

    def test_nested_insert_article_author_complex_view_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/nested_insert_article_author_complex_view_fail.yaml", transport)
...
class TestGraphqlUpdateBasic(DefaultTestMutations):
    ...
    def test_set_author_name(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_set_name.yaml", transport)

    def test_empty_set_author(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_empty_set.yaml", transport)
        hge_ctx.may_skip_test_teardown = True

    def test_set_person_details(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_set_details.yaml", transport)

    def test_person_id_inc(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_inc.yaml", transport)

    def test_no_operator_err(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_error_no_operator.yaml", transport)
...
class TestGraphqlUpdateJsonB(DefaultTestMutations):
    ...
    def test_jsonb_append_object(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_append_object.yaml", transport)

    def test_jsonb_append_array(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_append_array.yaml", transport)

    def test_jsonb_prepend_array(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_prepend_array.yaml", transport)

    def test_jsonb_delete_at_path(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_delete_at_path.yaml", transport)

    def test_jsonb_delete_array_element(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_delete_array_element.yaml", transport)

    def test_jsonb_delete_key(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/person_delete_key.yaml", transport)
...
class TestGraphqlUpdatePermissions(DefaultTestMutations):
    ...
    def test_user_can_update_unpublished_article(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/user_can_update_unpublished_article.yaml", transport)

    def test_user_cannot_update_published_version_col(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/user_cannot_update_published_article_version.yaml", transport)

    def test_user_cannot_update_another_users_article(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/user_cannot_update_another_users_article.yaml", transport)

    def test_user_cannot_update_id_col(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/user_cannot_update_id_col_article.yaml", transport)
...
class TestGraphqlDeleteConstraints(DefaultTestMutations):
    ...
    def test_author_delete_foreign_key_violation(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_foreign_key_violation.yaml", transport)
...
class TestGraphqlDeletePermissions(DefaultTestMutations):
    ...
    def test_author_can_delete_his_articles(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_can_delete_his_articles.yaml", transport)

    def test_author_cannot_delete_other_users_articles(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/author_cannot_delete_other_users_articles.yaml", transport)

    def test_resident_delete_without_select_perm_fail(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/resident_delete_without_select_perm_fail.yaml", transport)

    def test_agent_delete_perm_arr_sess_var(self, hge_ctx, transport):
        check_query_f(hge_ctx, self.dir() + "/agent_delete_perm_arr_sess_var.yaml", transport)

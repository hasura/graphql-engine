...
```bash
curl -L https://github.com/hasura/graphql-engine/raw/main/cli/get.sh | bash
```
...
```bash
curl -L https://github.com/hasura/graphql-engine/raw/main/cli/get.sh | INSTALL_PATH=$HOME/bin bash
```

You can also install a specific version of the CLI by providing the `VERSION` variable:

```bash
curl -L https://github.com/hasura/graphql-engine/raw/main/cli/get.sh | VERSION=v2.48.11 bash
```

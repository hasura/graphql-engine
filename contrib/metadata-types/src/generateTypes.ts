...
const config = load(readFileSync(configPath, 'utf-8')) as AppConfig;
...
for (const key in config.input_files) {
  ...
  config.input_files[key as SourceLanguage] =
    maybeConvertToMultiplePatterns(value);
}
...
async function jsonSchemaToYAML(filename: string) {
  ...
  const yamlSchema = dump(jsonSchema);
...
async function runTypeConversion(options: TypegenOptions) {
  ...
  return quicktype({ inputData, rendererOptions, lang: lang as any });
}

...
1. #### Endpoint
    ...
    The URL scheme (`http`/`https`) determines if SSL(TLS) should be used for the communication. Your APM vendor will
    provide the OpenTelemetry Ingestion endpoint information. The appropriate endpoint for the chosen **Connection Type**
...
4. #### Headers
    ...
    Headers are <em>(optionally)</em> added to every request made by Hasura to the APM receiver. APM vendors
    will provide **Authorization/API keys** with the required role/permissions to ingest OpenTelemetry data. Please refer to 
    their documentation for the request header formats. Headers should be added here as a `key:value` pair.

5. #### Attributes

    Attributes are <em>(optional)</em> custom tags added to the telemetry data. This is usually used to identify sources in a
    distributed tracing environment and aids in granular filtering and analytics.
...
data to the APM receiver. `Last Exported Traces` is continuously updated, indicating the timestamp of the last telemetry data that was successfully exported.
...
In case of errors while exporting telemetry data to the APM receiver, the dot becomes red and
...
Use your APM vendor's UI to to visualize and perform further analytics on trace data to monitor, diagnose and troubleshoot your application. Traces with their spans are listed out and selecting a trace shows a flame graph containing a visual representation of where
the operation spent its execution time. For example, the image below shows a flame graph in `Jaeger` where
...
data to the collector. Note: This collector should be run in your infrastructure and should be reachable by the Hasura backend, that is configured to send the data to the APM target. The list of [supported exporter targets](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter)

...
-   If your **password contains special characters** (e.g. #, %, $, @,
    ...
    You can check the [logs](/graphql/core/deployment/deployment-guides/kubernetes.mdx/) to see if the database
...
See [Setting up a GraphQL schema using an existing Postgres database](/graphql/core/databases/postgres/schema/using-existing-database.mdx#pg-schema-existing-db) to enable GraphQL over the database.

## Securing the GraphQL endpoint {#kubernetes_secure}
...
## Hasura GraphQL engine server logs {#kubernetes_logs}
...
## Updating Hasura GraphQL engine {#kubernetes_update}
...
<code>hasura/graphql-engine:v1.3.3</code>

    ...
    pub fn from_derive_input(input: &'a DeriveInput) -> syn::Result<Self> {
        ...
        let (json_schema_opts, data) = match &input.data {
            syn::Data::Struct(data) => {
                let struct_opts =
                    StructOpts::from_attributes(&input.attrs).expect("failed to parse struct opts");
                let struct_data = StructData::from_fields(&data.fields)?;
                (struct_opts.json_schema, Data::Struct(struct_data))
            }
            syn::Data::Enum(data) => {
                let enum_opts =
                    EnumOpts::from_attributes(&input.attrs).expect("failed to parse enum opts");
...
impl<'a> StructData<'a> {
    fn from_fields(fields: &'a syn::Fields) -> syn::Result<Self> {
        match fields {
...
impl<'a> NamedField<'a> {
    fn from_field(field: &'a syn::Field) -> Self {
        let (_, description) = super::helpers::get_title_and_desc_from_doc(&field.attrs);
        let field_type = &field.ty;
        let field_opts = FieldOpts::from_field(field).expect("failed to parse field opts");
...
impl<'a> EnumData<'a> {
    fn from_data(data: &'a syn::DataEnum, options: &EnumOpts) -> syn::Result<Self> {
...
impl<'a> EnumVariant<'a> {
    fn from_variant(variant: &'a syn::Variant, style: &EnumImplStyle) -> syn::Result<Self> {
        let variant_name = &variant.ident;
        let variant_opts =
            VariantOpts::from_attributes(&variant.attrs).expect("failed to parse variant opts");

...
var _ = Describe("hasura scripts update-project-v3", func() {
	...
	BeforeEach(func() {
		...
		stagingHasuraPort, teardownStaging := testutil.StartHasura(
			GinkgoT(),
	...
	It("update a config v2 project to config v3", func() {
		Context("sets up dev project", func() {
			...
			Expect(
				util.CopyDir("testdata/config-v2-test-project", projectDirectory),
			).Should(BeNil())
		...
		Context(
			"applies metadata and migrations on staging hasura instance with auto state migration disabled",
			func() {
				testutil.RunCommandAndSucceed(testutil.CmdOpts{
					Args:             []string{"metadata", "apply", "--endpoint", stagingEndpoint},
					WorkingDirectory: projectDirectory,
		...
		Context("applies metadata and migrations on staging hasura instance", func() {
			...
			).Should(ContainSubstring(`{"level":"debug","msg":"skipping state migration, found IsStateCopyCompleted: true Migrations: map[default:map[1620138136207:false 1620138146208:false 1620138161039:false 1620138169404:false 1620138179776:false 1620138189381:false 1620138199344:false]]"`))
		})

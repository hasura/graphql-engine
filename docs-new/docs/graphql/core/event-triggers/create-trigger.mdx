...
Add an event trigger by using the `pg_create_event_trigger` metadata API.
...
You can configure advanced settings via the `metadata_pg_create_event_trigger` metadata API.

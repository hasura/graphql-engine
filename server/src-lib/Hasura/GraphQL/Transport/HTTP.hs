...
runActionExecution
  ...
  -> ET.ActionExecution
  -> ExceptT (Either a QErr) m ResultsFragment
runActionExecution httpHeaders (ET.ActionExecution action) = do
  (telemTimeIO_DT, resp) <- withExceptT Right $ withElapsedTime action
  pure $ ResultsFragment telemTimeIO_DT Telem.Local resp httpHeaders
    ...
    (telemCacheHit,) <$> case execPlan of
        ...
        case cachedValue of
          ...
          Nothing -> do
            conclusion <- runExceptT $ forWithKey queryPlans $ \fieldName -> \case
              ...
              E.ExecStepAction action ->
                runActionExecution [] action
              E.ExecStepRaw json ->
                buildRaw json
      ...
      E.MutationExecutionPlan mutationPlans -> do
        conclusion <- runExceptT $ forWithKey mutationPlans $ \fieldName -> \case
          ...
          E.ExecStepRemote (rsi, opDef, varValsM) ->
            runRemoteGQ fieldName rsi opDef varValsM
          E.ExecStepAction (action, headers) ->
            runActionExecution headers action
          E.ExecStepRaw json ->
            buildRaw json

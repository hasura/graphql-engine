...
func TestOpentelemetryConfig_Build(t *testing.T) {
	type fields struct {
		MetadataDir string
...
func TestBackendConfigsConfig_Export(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			map[string][]byte{
				"metadata/opentelemetry.yaml": func() []byte {
					bs, err := ioutil.ReadFile("testdata/export_test/t1/want.opentelemetry.yaml")
					assert.NoError(t, err)
					return bs
				}(),
			},

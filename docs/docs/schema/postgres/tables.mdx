...
2.  Add the following statement to the `down.sql` file in case you need
    to [roll back](/migrations-metadata-seeds/manage-migrations.mdx#roll-back-migrations)` the above statement:
...
following API call to the [pg_track_table](/api-reference/metadata-api/table-view.mdx#metadata-pg-track-table) metadata API:
...
See the [query](/api-reference/graphql-api/query.mdx) and
[mutation](/api-reference/graphql-api/mutation.mdx) API references for the full
specifications.

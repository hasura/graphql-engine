  ...
  remoteSchemaPermsCtx =
    do
      ...
      runMonadSchema role roleQueryContext sources $ do
        ...
        subscriptionParser <-
          buildSubscriptionParser subscriptionFields allActionInfos customTypes subscriptionRemotes
        queryParserFrontend <-
...
buildRelayRoleContext
  ...
  role = do
    ...
    runMonadSchema role roleQueryContext sources do
      ...
      subscriptionParser <-
        buildSubscriptionParser queryPGFields [] customTypes mempty
      queryParserFrontend <-
...
buildFullestDBSchema queryContext sources allActionInfos customTypes =
  runMonadSchema adminRoleName queryContext sources do
    ...
    mutationParserFrontend <-
      ...
      buildMutationParser mempty allActionInfos customTypes mutationFrontendFields
    subscriptionParser <-
      buildSubscriptionParser subscriptionFields allActionInfos customTypes mempty
    queryParserFrontend <-
...
buildTableStreamSubscriptionFields sourceName sourceConfig tables queryTagsConfig = do
  tableSelectExpParsers <- for (Map.toList tables) \(tableName, tableInfo) -> do
    ...
    selectPerms <- tableSelectPermissions tableInfo
    for selectPerms $
      mkRF . buildTableStreamingSubscriptionFields sourceName sourceConfig queryTagsConfig tableName tableInfo tableGQLName
  pure $ concat $ catMaybes tableSelectExpParsers
  where
    mkRF = mkRootField sourceName sourceConfig queryTagsConfig (Just StreamingSubscription) QDBR
...
buildMutationFields scenario sourceName sourceConfig tables (takeExposedAs FEAMutation -> functions) queryTagsConfig = do
  ...
  where
    ...
    mkRF = mkRootField sourceName sourceConfig queryTagsConfig Nothing
...
mkRootField ::
  ...
  SourceConfig b ->
  Maybe QueryTagsConfig ->
  Maybe StreamingSubscription ->
  (a -> db b) ->
  m [FieldParser n a] ->
  m [FieldParser n (RootField db remote action raw)]
mkRootField sourceName sourceConfig queryTagsConfig streamSubsMaybe inj =
  fmap
    ( map
        ( fmap
            ( RFDB sourceName streamSubsMaybe
                . AB.mkAnyBackend @b

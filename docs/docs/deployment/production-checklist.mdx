...
Hasura does [connection pooling](https://hasura.io/docs/latest/databases/database-config/cloud-connection-pooling/) for
Postgres and MS SQL Server databases. You should set an appropriate [total_max_connections](/api-reference/syntax-defs/#pgpoolsettings) (on Cloud) or [max_connections](/api-reference/syntax-defs/#pgpoolsettings) 
(on self-hosted) to utilize the database efficiently.

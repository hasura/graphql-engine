...
type InconsistentMetadataError struct {
	Definition any    `json:"definition,omitempty" mapstructure:"definition,omitempty"`
...
func (h HasuraError) Error() string {
	...
	var (
		internalError  SQLInternalError
		internalErrors []SQLInternalError
	)

	if v, ok := h.Internal.(map[string]any); ok {
		...
		if err == nil {
			...
			if internalError.Error != nil {
				errorStrings = append(
					...
					fmt.Sprintf(
						"[%s] %s: %s",
						internalError.Error.StatusCode,
						internalError.Error.ExecStatus,
				...
				if len(internalError.Error.Description) > 0 {
					errorStrings = append(
						errorStrings,
						"Description: "+internalError.Error.Description,
					)
				}

				if len(internalError.Error.Hint) > 0 {
					errorStrings = append(errorStrings, "Hint: "+internalError.Error.Hint)
				}
			}

			if e := internalError.String(); e != "" {
				errorStrings = append(errorStrings, e)
			}
		}
	}

	if v, ok := h.Internal.([]any); ok {

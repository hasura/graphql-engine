...
func TestMetadataCmd(t *testing.T, ec *cli.ExecutionContext) {
	...
	tt := []struct {
	...
	}{
		{
			...
			nil,
			"",
			filepath.Join(currDir, "v1/metadata/1_metadata.yaml"),
		},
		{
			"metadata-apply",
			...
			nil,
			filepath.Join(currDir, "v1/metadata/2_metadata.yaml"),
			"",
		},
		{
			"metadata-export",
		...
		},
		{
			...
			&commands.MetadataDiffOptions{
				EC:     ec,
				Args:   []string{filepath.Join(currDir, "v1/metadata/2_metadata.yaml")},

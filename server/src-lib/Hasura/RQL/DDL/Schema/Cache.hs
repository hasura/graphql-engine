...
mergeCustomTypes gCtxMap remoteSchemaCtx customTypes = do
  let commonTypes = M.intersectionWith (,) existingTypes customTypes
  ...
  return ( fmap addCustomTypes gCtxMap
         , addCustomTypes remoteSchemaCtx
         )
  where
    addCustomTypes gCtx =

...
:::note Datadog Integration is only available for Hasura Cloud projects on the `Standard` (pay-as-you-go) tier and above. :::
...
:::note Datadog allows ingestion of logs with maximum size 256kB for a single log. If a log exceeds this limit, Datadog will truncate the log at 256kB. :::

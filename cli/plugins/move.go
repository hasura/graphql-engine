...
func findMoveTargets(fromDir, toDir string, fo FileOperation) ([]move, error) {
	...
	if fo.To != filepath.Clean(fo.To) {
		return nil, errors.E(
			op,
			fmt.Errorf(
				"the provided path is not clean, %q should be %q",
				fo.To,
				filepath.Clean(fo.To),
			),
		)
	}
	...
	if err != nil {
		return nil, errors.E(
			op,
			fmt.Errorf("could not get the relative path for the move src: %w", err),
		)
	}

	m, ok, err := getDirectMove(fromDir, toDir, fo)
	if err != nil {
...
func getDirectMove(fromDir, toDir string, fo FileOperation) (move, bool, error) {
	...
	if err != nil {
		return m, false, nil //nolint:nilerr
	}

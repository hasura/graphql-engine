...
-- <<<<<<< HEAD:server/src-lib/Hasura/RQL/DML/RemoteJoin.hs
-- =======
-- import           Control.Lens
...
-- >>>>>>> ce7e8288... server: query remote server in a remote join query iff all arguments are not null (#31):server/src-lib/Hasura/Backends/Postgres/Execute/RemoteJoin.hs
...
traverseQueryResponseJSON
  :: (MonadError QErr m)
  => RemoteJoinMap -> AO.Value -> m (CompositeValue (Maybe RemoteJoinField))

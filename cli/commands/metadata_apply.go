...
`,
  ...
  hasura metadata apply --disallow-inconsistent-metadata`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if opts.FromFile {
				return errors.E(op, stderrors.New("use of deprecated flag"))
			}
	...
	f.BoolVar(
		...
		"from-file",
		false,
		"apply metadata from migrations/metadata.[yaml|json]",
	)

	err := f.MarkDeprecated(
		"from-file",
		"deprecation is a side effect of config v1 deprecation from v2.0.0",
	)
	if err != nil {
	...
	f.StringVarP(
		...
		"o",
		"",
		`specify an output format to show applied metadata. Allowed values: json, yaml (default "json")`,
	)
	f.BoolVar(
		&opts.DisallowInconsistencies,
		"disallow-inconsistent-metadata",
		false,
		"disallow inconsistent metadata to be applied. Defaults to false",
	)

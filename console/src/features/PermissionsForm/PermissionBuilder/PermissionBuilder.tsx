...
import QueryBuilderJson from '../../../components/Common/QueryBuilderJson/QueryBuilderJson';
import {
  getAllJsonPaths,
  isArray,
  isJsonString,
...
class PermissionBuilder extends React.Component<PermissionBuilderProps> {
  ...
  loadMissingSchemas(
    ...
    const findMissingSchemas = (
      ...
      } else if (isColumnOperator(operator as ColumnOperators)) {
        // no missing schemas
      } else {
        const { allTableSchemas } = this.props;
  ...
  render() {
    ...
    const getFilter = (
      ...
      if (operator === '') {
        // blank bool exp
      } else if (isArrayBoolOperator(operator as BoolOperators)) {
    ...
    const renderOperatorExp = (
      ...
      if (operator) {
        const operatorInputType =
          getOperatorInputType(operator as ColumnOperators) || valueType;
    ...
    const renderBoolExp = (
      ...
      if (operation) {
        ...
        if (isArrayBoolOperator(operation as BoolOperators)) {
        ...
        } else if (isBoolOperator(operation as BoolOperators)) {

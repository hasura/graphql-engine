...
This can be achieved by :doc:`adding a new column <alter-schema/alter-column>` called ``user_id`` (or any other name of your choice) of type ``integer`` to the table and storing the ``hasura_id`` of the user in it while adding a row.

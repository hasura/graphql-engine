...
This is necessary to ensure [nested inserts](/guides/data-modelling/one-to-one.mdx#one-to-one-insert) in one-to-one
relationships are possible using either side as the parent which would otherwise error out with a `Not-NULL violation`
...
Let's say the `articles` has a [many-to-many relationship](/guides/data-modelling/many-to-many.mdx) with the `tags`

...
Listing tables might be a bit of a complex part in the migration.

This section probably needs a direct migration from RST to MDX rather than from `pandoc` migrated files as the auto migration messes up the code block syntax with lots of `span`'s. Apologies for the inconvenience.
...
If a table cell is mixed with text and markdown stuff (`code highlighters`, [links](/rst-vs-mdx-guide/links) etc., Please prefer `html` syntax over markdown as markdown won't be parsed correctly.

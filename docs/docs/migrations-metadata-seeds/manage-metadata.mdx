...
## Introduction
We call the Hasura Server configuration the Metadata. All changes made to the Hasura instance via the Console or via
the API such as tracking tables / views / functions, creating relationships, configuring permissions, creating event
...
another Hasura instance to get the same configuration. You can also manually edit the Metadata and then use it to
update the instance.

If you have [already initialized](/hasura-cli/commands/hasura_init.mdx) your project via the Hasura CLI you
should see the Metadata directory structure in your project directory. If you are only using the Hasura Console from
the Hasura CLI then it will be kept fully updated as you make changes. However, you can always create a manual
export if you need to make sure that it is up-to-date.
...
There are three ways to interact with the Hasura Metadata. Via the [Hasura CLI](/hasura-cli/index.mdx),
the Hasura Console, and the Hasura API. We'll go over each Metadata command below and how it is executed with each
method.

### Metadata Formats
Internally, Hasura saves Metadata as a single **JSON** blob in a table called `hdb_metadata` in a schema called
`hdb_catalog` in the database which you've designated as your Metadata database. Metadata can be exported and
imported in this JSON blob format using the Console or Hasura API. Using the Hasura CLI, the Metadata is exported
and managed in **YAML** format using separate files for each Metadata type.
...
To export your Metadata means to save your internal Hasura Metadata configuration to a file or group of files in
order to track it with version control or use it in a CI/CD pipeline.
...
<TabItem value="console" label="Console">
  ...
  <Thumbnail
    ...
    src="/img/graphql/core/migrations/settings-navigation_console_2-7-0.png"
  />

  2. In the Hasura Metadata actions page that opens, click on the `Export Metadata` button.

  <Thumbnail
    src="/img/graphql/core/migrations/metadata-export_console_2-7-0.png"
    alt="Export metadata"
  />
...
<TabItem value="api" label="API">
  ...
  This command will create a `hasura_metadata.json` file. If an admin secret is set, add `-H 'X-Hasura-Admin-Secret:
  [your-admin-secret]'` as the API is an admin-only API.
...
You can apply Metadata from one Hasura Server instance to another. You can also apply an older or modified version
of an instance's Metadata onto itself to replace the existing Metadata.

Applying or importing completely replaces the Metadata on that instance, i.e. you lose any Metadata that existed
before applying.
...
<TabItem value="console" label="Console">
  ...
  <Thumbnail
    src="/img/graphql/core/migrations/metadata-import_console_2-7-0.png"
    alt="Import metadata"
  />
...
<TabItem value="api" label="API">
  ...
  [replace_metadata](/api-reference/metadata-api/manage-metadata.mdx#metadata-replace-metadata)
  metadata API.
  ...
  This command reads the `hasura_metadata.json` file and makes a POST request to replace the Metadata. If an admin
  secret is set, add `-H 'X-Hasura-Admin-Secret: [your-admin-secret]'` as the API is an admin-only API.
...
[apply the database Migration](/migrations-metadata-seeds/manage-migrations.mdx#apply-migrations)
schema first, before apply the Metadata.

:::

## Roll back Hasura Metadata {#rollback-metadata}

As Hasura metadata is Managed via snapshots of whole of the Metadata, to roll it back to a particular state you need
the Metadata snapshot at that point which you would typically achieve by checking out stable checkpoints of a
project in version control.

## Reload Hasura Metadata {#reload-metadata}

In some cases, the Metadata can be out of sync with the database schema. For example, when a new column has been
added to a table via an external tool such as `psql`.
...
<TabItem value="console" label="Console">
  ...
  <Thumbnail
    alt="reload metadata"
    src="/img/graphql/core/migrations/metadata-reload_console_2-7-0.png"
  />
...
[resolve all inconsistent objects](/migrations-metadata-seeds/manage-metadata.mdx#metadata-inconsistency)
manually or delete them. After that, you will need to reload metadata again.
...
reapplied if necessary or else that information will be lost and Hasura will have to be configured again
...
<TabItem value="api" label="API">
  ...
  [clear_metadata](/api-reference/metadata-api/manage-metadata.mdx#metadata-clear-metadata)
  Metadata API.
...
automatically apply Migrations and Metadata when the Hasura Server starts.

...
mergeCustomTypes
  :: MonadError QErr f
  => M.HashMap Role GS.GCtx -> GS.GCtx -> (NonObjectTypeMap, AnnotatedObjects)
...
mergeCustomTypes gCtxMap remoteSchemaCtx customTypesState = do
  ...
  let gCtxMapWithCustomTypes = flip M.mapWithKey gCtxMap $ \role gCtx ->
        let customTypes = buildCustomTypesSchema (fst customTypesState)
                          (snd customTypesState) role

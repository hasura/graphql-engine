...
func (re *RestEndpointsConfig) CreateFiles() error {
	...
	err = os.WriteFile(filepath.Join(re.MetadataDir, re.Filename()), buf.Bytes(), 0o644)
	if err != nil {
		return errors.E(op, err)
	}
...
func (re *RestEndpointsConfig) Build() (map[string]any, error) {
	...
	return map[string]any{re.Key(): obj}, nil
}
...
func (re *RestEndpointsConfig) WriteDiff(opts metadataobject.WriteDiffOpts) error {
	...
	err := metadataobject.DefaultWriteDiff(
		metadataobject.DefaultWriteDiffOpts{From: re, WriteDiffOpts: opts},
	)
	if err != nil {
...
func (re *RestEndpointsConfig) error(
	err error,

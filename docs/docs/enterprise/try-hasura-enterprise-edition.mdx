...
In some environments it may not be enough to run the `restart` command. In such cases, you may need to stop and
start the container with `docker compose down` and `docker compose up -d` respectively.

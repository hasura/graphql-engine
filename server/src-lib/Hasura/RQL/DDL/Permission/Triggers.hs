...
buildInsTrigFn :: QualifiedTable -> QualifiedTable -> S.BoolExp -> S.SetExp -> Q.Query
buildInsTrigFn fn tn be se =
  Q.fromBuilder $ mconcat
  ...
  , BB.string7 " VALUES ($1.*) ON CONFLICT ON CONSTRAINT ' || constraint_name || ' DO UPDATE " <> toSQL se

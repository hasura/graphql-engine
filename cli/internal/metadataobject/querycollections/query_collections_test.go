...
func TestQueryCollectionConfig_Export(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			map[string][]byte{
				"metadata/query_collections.yaml": func() []byte {
					bs, err := os.ReadFile("testdata/export_test/t1/want.query_collections.yaml")
					assert.NoError(t, err)
					return bs
				}(),
			},
		...
		},
		{
			...
			map[string][]byte{
				"metadata/query_collections.yaml": func() []byte {
					bs, err := os.ReadFile("testdata/export_test/t2/want.query_collections.yaml")
					assert.NoError(t, err)
					return bs
				}(),
			},
		...
		},
		{
			...
			map[string][]byte{
				"metadata/query_collections.yaml": func() []byte {
					bs, err := os.ReadFile("testdata/export_test/t3/want.query_collections.yaml")
					assert.NoError(t, err)
					return bs
				}(),
			},
		...
		},
		{
			...
			map[string][]byte{
				"metadata/query_collections.yaml": func() []byte {
					bs, err := os.ReadFile("testdata/export_test/t4/want.query_collections.yaml")
					assert.NoError(t, err)
					return bs
				}(),
			},
		...
		},
		{
			...
			map[string][]byte{
				"metadata/query_collections.yaml": readYamlFileAndEmitBytes(
					t,
					"testdata/export_test/t5/want.query_collections.yaml",
				),
			},
			false,
			require.NoError,
...
func readJsonFileAndEmitYamlNode(t *testing.T, jsonFilePath string) map[string]yaml.Node {
	t.Helper()
	bs, err := os.ReadFile(jsonFilePath)
	assert.NoError(t, err)
	yamlbs, err := metadatautil.JSONToYAML(bs)
	assert.NoError(t, err)
	var v map[string]yaml.Node

...
func MigrateAPI(c *gin.Context) {
	...
	if !ok {
		c.JSON(
			...
			&Response{Code: "internal_error", Message: "cannot get execution context"},
		)
	...
	case http.MethodGet:
		...
		if ec.Config.Version >= cli.V3 {
			kind, err := metadatautil.GetSourceKind(
			...
			if err != nil {
				c.JSON(
					http.StatusInternalServerError,
					&Response{Code: "internal_error", Message: err.Error()},
				)
			...
			if kind == nil {
				c.JSON(
					...
					&Response{
						...
						Message: fmt.Sprintf("cannot determine database kind for '%v'", sourceName),
					},
				)
		...
		if err != nil {
			c.JSON(
				http.StatusInternalServerError,
				&Response{Code: "internal_error", Message: "Something went wrong"},
			)
	...
	case http.MethodPost:
		...
		if err = c.BindJSON(&request); err != nil {
			c.JSON(
				http.StatusInternalServerError,
				&Response{Code: "request_parse_error", Message: err.Error()},
			)
		...
		if ec.Config.Version >= cli.V3 && request.SourceName == "" {
			c.JSON(
				http.StatusInternalServerError,
				&Response{Code: "internal_error", Message: "datasource key not found in body"},
			)
			...
			if kind == nil && len(request.Up) == 0 {
				c.JSON(
					...
					&Response{
						Code: "request_parse_error",
						Message: fmt.Sprintf(
							"didn't find any request in UP in query body %v",
							sourceName,
						),
					},
				)
				...
				default:
					c.JSON(
						...
						&Response{
							Code: "request_parse_error",
							Message: fmt.Sprintf(
								"cannot determine database kind for '%v'",
								sourceName,
							),
						},
					)
		...
		createOptions := cmd.New(
			timestamp,
			request.Name,
			filepath.Join(ec.MigrationDir, sourceName),
		)
		sqlUp := &bytes.Buffer{}
		...
		if sqlUp.String() != "" {
			...
			if err != nil {
				c.JSON(
					http.StatusInternalServerError,
					&Response{Code: "create_file_error", Message: err.Error()},
				)
		...
		if err = t.QueryWithVersion(
			uint64(timestamp),
			io.NopCloser(bytes.NewReader(upByt)),
			request.SkipExecution,
		); err != nil {
			if after, ok0 := strings.CutPrefix(err.Error(), DataAPIError); ok0 {
				c.JSON(http.StatusBadRequest, &Response{Code: "data_api_error", Message: after})

				return
			}

			if errors.Is(err, migrate.ErrNoMigrationMode) {
				c.JSON(
					http.StatusBadRequest,
					&Response{Code: "migration_mode_disabled", Message: err.Error()},
				)

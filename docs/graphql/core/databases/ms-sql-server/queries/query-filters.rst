...
The ``_eq`` (equal to) or the ``_neq`` (not equal to) operators are compatible with any MS SQL Server type (like ``Integer``, ``Float``, ``Double``, ``Text``, ``Boolean``,
...
``_lte`` (less than or equal to) operators are compatible with any MS SQL Server type 
...
They are compatible with any MS SQL Server type (like ``Integer``, ``Float``, ``Double``,
...
``_st_within`` operators are used to filter based on ``geometry`` like columns.
...
.. _ms_sql_server_true_expression:
...
.. _ms_sql_server_null_value_evaluation:

Evaluation of **null** values in comparison expressions
-------------------------------------------------------

If in any comparison expression a ``null`` value is passed, a type mismatch error
will be thrown.

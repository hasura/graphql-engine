...
benchWsApp :: GraphQLURL -> TMVar.TMVar () -> WS.ServerApp
benchWsApp graphqlUrl lock pending = do
 conn <- WS.acceptRequest pending
 -- WS.withPingThread conn 30 (return ()) $
 forever $ do
     msg <- WS.receiveData conn
     fork_ $ processReq conn msg graphqlUrl lock
 where
   fork_ = void . forkIO

processReq :: WS.Connection -> BLC.ByteString -> GraphQLURL -> TMVar.TMVar () -> IO ()
processReq conn msg graphqlURL lock = do
  resE <- catchIO $ runExceptT $ processReq' conn msg graphqlURL lock
...
processReq' :: (MonadIO m, MonadError ErrorMessage m)
  => WS.Connection -> BL.ByteString -> GraphQLURL -> TMVar.TMVar ()
  -> m (BenchConf, BenchResult)
processReq' conn msg graphqlURL lock = do
  conf <- parseIncomingMsg
  res <- withLock $ runBench conn conf graphqlURL
  return (conf, res)
  where
...
runBench :: (MonadIO m, MonadError ErrorMessage m)
  => WS.Connection -> BenchConf -> GraphQLURL -> m BenchResult
runBench conn conf graphqlURL = do
  sendStartMsg
  result <- case conf of
    BCWrk args  -> BRWrk <$> runWrkBench args graphqlURL
    BCWrk2 args -> BRWrk2 <$> runWrk2Bench args graphqlURL
  void sendFinishMsg
  return result
  where
...
runWrkBench :: (MonadIO m, MonadError ErrorMessage m)
 => WrkBenchArgs -> GraphQLURL -> m WrkResultOut
runWrkBench args@WrkBenchArgs{..} graphqlURL  = do
   ...
   where
     runQueryOnce = runQuery graphqlURL wbaQuery
     wrkResult stderr = jsonDecode $ BLC.pack stderr
     wrkArgs script = toArgsList (/="query") args <> luaScriptArgs script
     luaScriptArgs script = ["-s", show script, graphqlURL, T.unpack $ getQuery wbaQuery]
...
runWrk2Bench :: (MonadIO m, MonadError ErrorMessage m)
  => Wrk2BenchArgs -> GraphQLURL -> m Wrk2ResultOut
runWrk2Bench args@Wrk2BenchArgs{..} graphqlURL = do
  _ <- liftIO runQueryOnce -- run the GraphQL query once to ensure there are no errors
  ...
  case exitCode of
   ...
   where
     runQueryOnce = runQuery graphqlURL w2baQuery
     ...
     wrk2Args script = toArgsList (/= "query") args <> ["--latency"] <> luaScriptArgs script
     luaScriptArgs script = ["-s", show script] <> [ graphqlURL, query, resultsDir ]

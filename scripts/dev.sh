...
if [ "$MODE" = "graphql-engine" ]; then
  ...
  function cleanup {
    ...
    if command -v hpc >/dev/null; then
      ...
      hpcdir=$(ls -td dist-newstyle/build/*/ghc-*/graphql-engine-*/noopt/hpc/vanilla/mix/graphql-engine-*  dist-newstyle/build/*/ghc-*/graphql-engine-*/hpc/vanilla/mix/graphql-engine-* | head -1)

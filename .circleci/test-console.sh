...
if [ "$TEST_SUITE_NUMBER" = "1" ]; then
  ...
  docker logs test1 > /home/circleci/build/_console_test_logs/1.log
  EXIT_CODE_2="$(docker wait test2)"
  docker logs test2 > /home/circleci/build/_console_test_logs/2.log
  if [ "$EXIT_CODE_1" != "0" ] || [ "$EXIT_CODE_2" != "0" ]; then
    exit 1
  fi
fi
if [ "$TEST_SUITE_NUMBER" = "2" ]; then
  docker run --name migration_mode -e TEST_ENV=MIGRATE_URL=http://${CONSOLE_HOST}:9693/apis/migrate -e TEST_CONFIG=baseUrl=http://${CONSOLE_HOST}:3000 -e 'TEST_SPEC=cypress/integration/data/migration-mode/test.js' -d -v /home/circleci/graphql-engine:/root/graphql-engine hasura/graphql-engine-console-builder:v0.3 /bin/bash -c "cd /root/graphql-engine && ./.circleci/run-console-test.sh"
  ...
  docker logs test3 > /home/circleci/build/_console_test_logs/3.log
  docker wait test4
  EXIT_CODE_4="$(docker wait test4)"
  docker logs test4 > /home/circleci/build/_console_test_logs/4.log

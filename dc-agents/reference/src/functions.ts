...
export function respondToFunction(
  functionName: FunctionName,
  functionArgs: FunctionRequestArgument[],
  getTable: GetTable,
): Rows {
...
const functions: Record<
  string,
  (functionArgs: FunctionRequestArgument[], getTable: GetTable) => Rows
> = {
  '[Fibonacci]': fibonacci,
...
function fibonacci(functionArgs: FunctionRequestArgument[]): Rows {
  ...
  if (!n) {
    throw Error('Expecting `take` arg');
  }
...
function search_articles(
  ...
  if (!functionArgs) {
    throw Error('Expecting function_arguments');
  }
  const argz = namedArguments(functionArgs);

  const table = getTable(['Articles']);
  if (!table) {
    throw Error('Could not find `Articles` table.');
  }

  const q = argz['query']?.value;
  if (!q) {
    throw Error('Expecting `query` arg');
  }

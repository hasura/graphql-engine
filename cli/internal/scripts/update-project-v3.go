...
func removeDirectories(fs afero.Fs, parentDirectory string, dirNames []string) error {
	...
	for _, d := range dirNames {
		err := fs.RemoveAll(filepath.Join(parentDirectory, d))
		if err != nil {
			return errors.E(op, err)
		}
	}
...
func getSeedFiles(fs afero.Fs, rootSeedDir string) ([]string, error) {
	...
	for _, info := range dirs {
		if !info.IsDir() {
			seedFiles = append(seedFiles, filepath.Clean(info.Name()))
		}
	}
...
func getMatchingFilesAndDirs(
	fs afero.Fs,
	parentDir string,
	matcher func(string) (bool, error),
) ([]string, error) {
	...
	for _, info := range dirs {
		...
		migs = append(migs, filepath.Clean(info.Name()))
	}
...
func CopyState(ec *cli.ExecutionContext, sourceDatabase, destDatabase string) error {
	...
	src := migrations.NewMigrationStateStoreHdbTable(
...
func CheckIfUpdateToConfigV3IsRequired(ec *cli.ExecutionContext) error {
	...
	if ec.Config.Version < cli.V3 && ec.HasMetadataV3 {
		...
		upgrade := func() error {
			...
			ec.Logger.Info(
				"please use " + color.New(color.FgCyan).
					SprintFunc()(

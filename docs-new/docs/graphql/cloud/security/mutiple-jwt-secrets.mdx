...
Bearer Tokens are authenticated against the secret with a matching or absent <span class="title-ref">issuer</span>.
...
2. Tokens are filtered to ensure that the <span class="title-ref">issuer</span> field matches, or that the issuer is absent either from the configuration, or the token.

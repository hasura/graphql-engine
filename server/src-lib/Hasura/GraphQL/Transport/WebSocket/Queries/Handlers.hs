...
onConnHandler :: L.Logger -> CorsPolicy -> WS.OnConnH ConnState
...
onConnInit
  :: L.Logger -> H.Manager -> WSConn -> WSConnData -> AuthMode -> Maybe ConnParams -> IO ()
  ...
  where
    ...
    runHasuraGQ reqId query userInfo = \case
      ...
      E.ExOpSubs lqOp -> do
        ...
        liftIO $ L.unLogger logger $ QueryLog query Nothing reqId
    ...
    execQueryOrMut reqId query genSql action = do
      ...
      liftIO $ L.unLogger logger $ QueryLog query genSql reqId
...
logWSEvent
  :: MonadIO m => L.Logger -> WSConn -> WSConnData -> WSEvent -> m ()

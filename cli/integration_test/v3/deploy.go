	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedMetadataFolder != "" {
				...
				err = filepath.Walk(
					filepath.Join(tc.expectedMetadataFolder),
					func(path string, info os.FileInfo, err error) error {
						if !info.IsDir() {
							name := info.Name()

							expectedByt, err := os.ReadFile(path)
							if err != nil {
								t.Fatalf(
							...
							actualByt, err := os.ReadFile(
								strings.Replace(path, tc.expectedMetadataFolder, ec.MetadataDir, 1),
							)
							if err != nil {
								t.Fatalf(

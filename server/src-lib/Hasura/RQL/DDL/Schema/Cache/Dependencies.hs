...
pruneDanglingDependents cache = fmap (M.filter (not . null)) . traverse do
  ...
  where
    ...
    resolveDependency (SchemaDependency objectId _) = case objectId of
      SOSourceObj source sourceObjId -> case sourceObjId of
        ...
        SOIFunction functionName -> void $
          (M.lookup source (_boSources cache) >>= M.lookup functionName . _soFunctions)
    ...
    resolveTable source tableName =
      (M.lookup source (_boSources cache) >>= M.lookup tableName . _soTables)
...
deleteMetadataObject objectId = case objectId of
  ...
  MOSourceObjId source sourceObjId -> boSources.ix source %~ case sourceObjId of
    SMOTable        name -> soTables        %~ M.delete name
    SMOFunction     name -> soFunctions     %~ M.delete name
    SMOTableObj tableName tableObjectId -> soTables.ix tableName %~ case tableObjectId of

...
def default_env(path, app_name):
    ...
    set_key(path, 'DATABASE_PASS', 'gql_test')
    set_key(path, 'DATABASE_URL', 'postgres://gql_test:gql_test@localhost:5432/{}'.format(app_name))

POSTGRES_DEFAULT_CMD=['PGPASSWORD=gql_test', 'psql', '-h', 'localhost', '-p', '5432', '-U', 'gql_test', '-c']

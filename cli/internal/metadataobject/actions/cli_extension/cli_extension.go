...
func (c *Config) ConvertMetadataToSDL(toPayload types.SDLToRequest) (types.SDLToResponse, error) {
	var (
		op         errors.Op = "cliextension.Config.ConvertMetadataToSDL"
		toResponse types.SDLToResponse
	)

	outputFile, err := os.CreateTemp("", "*.json")
	if err != nil {
...
func (c *Config) ConvertSDLToMetadata(
...
) (types.SDLFromResponse, error) {
	var (
		op           errors.Op = "cliextension.Config.ConvertSDLToMetadata"
		fromResponse types.SDLFromResponse
	)
	...
	c.logger.WithField("command", "sdl from").Debugln("output: " + stdout.String())
...
func (c *Config) GetActionsCodegen(
	codegenReq types.ActionsCodegenRequest,
) (types.ActionsCodegenResponse, error) {
	var (
		op          errors.Op = "cliextension.Config.GetActionsCodegen"
		codegenResp types.ActionsCodegenResponse
	)
	...
	c.logger.WithField("command", "actions-codegen").Debugln("output: " + stdout.String())

...
func (a *ActionConfig) CreateFiles() error {
	...
	err = os.WriteFile(filepath.Join(a.MetadataDir, graphqlFileName), graphqQLData, 0o644)
	if err != nil {
		return errors.E(op, err)
	}
...
func (a *ActionConfig) Build() (map[string]any, error) {
	var op errors.Op = "actions.ActionConfig.Build"

	if !a.serverFeatureFlags.HasAction {
		_, err := a.GetActionsFileContent()
		if err == nil {
			a.logger.WithField("metadata_plugin", "actions").
				Warnf("Skipping building %s", a.Filename())
		}

		_, err = a.GetActionsGraphQLFileContent()
		if err == nil {
			a.logger.WithField("metadata_plugin", "actions").
				Warnf("Skipping building %s", graphqlFileName)
		}
	...
	if err != nil {
		return nil, errors.E(
			op,
			a.error(fmt.Errorf("error in reading %s file: %w", graphqlFileName, err)),
		)
	}
	...
	customTypesLen := len(
		sdlFromResp.Types.Enums,
	) + len(
		sdlFromResp.Types.InputObjects,
	) + len(
		sdlFromResp.Types.Objects,
	) + len(
		sdlFromResp.Types.Scalars,
	)
	if customTypesLen != 0 {
...
func (a *ActionConfig) getActionsCodegenURI(framework string) string {
	return fmt.Sprintf(
		`https://raw.githubusercontent.com/%s/master/%s/actions-codegen.js`,
		util.ActionsCodegenOrg,
		framework,
	)
}

func (a *ActionConfig) error(
	err error,

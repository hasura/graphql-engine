...
const { getAstTypeMetadata, wrapTypename } = require('../../utils/wrappingTypeUtils');
const { parseCustomTypes, reformCustomTypes, hydrateTypeRelationships } = require('../../utils/typeUtils')

const getTypesFromSdl = sdl => {
...
const getActionDefinitionFromSdl = sdl => {
  const schemaAst = sdlParse(sdl);
  const definition = {
    name: '',
    arguments: [],
...
const getTypesSdl = _types => {

  let types;
  if (_types.constructor.name !== "Array") {
...
const getServerTypesFromSdl = (sdl, existingTypes) => {

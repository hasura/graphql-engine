...
Hasura Cloud provides support for caching query responses, in order to improve performance for queries that are executed frequently.
...
:::info Support Query response caching is available for Hasura Cloud projects on the `Standard` (pay-as-you-go) tier and above. :::
...
If the response was cached successfully, the HTTP response will include a `X-Hasura-TTL` header, whose value indicates the maximum number of seconds for the returned response to remain in the cache.

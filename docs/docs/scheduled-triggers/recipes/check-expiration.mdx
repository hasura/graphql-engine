...
def get_expired_coupons():
    ...
    headers = {
        'Content-Type': 'application/json',
        'x-hasura-admin-secret': 'super_secret_string_123',
    }

    graphql_query = '''
    query GetExpiringCoupons($two_days_from_now: timestamptz!) {
      coupons_coupon(where: {expiration_date: {_lte: $two_days_from_now}}) {
        id
        expiration_date
        code
        user_data {
    ...
    response = requests.post('http://your-graphql-endpoint', json=payload, headers=headers)
...
def expiration_check():
    ...
    for coupon in expired_coupons:
        ...
        msg['To'] = f"{coupon['user_data']['name']} <{email}>"
        msg['Subject'] = f"You've got a coupon expiring soon, {coupon['user_data']['name'].split(' ')[0]}!"
        message_body = f"Yo {coupon['user_data']['name'].split(' ')[0]},\n\nYour coupon code, {coupon['code']}, is expiring soon! Use it before {readable_date}.\n\nThanks,\nSuperStore.com"

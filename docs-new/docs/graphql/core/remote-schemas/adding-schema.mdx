...
# Adding a remote schema{#adding-schema}
...
## Step 1: Merge remote schema {#merge_remote_schema}
...
You can add a remote schema by using the [add_remote_schema](/graphql/core/api-reference/metadata-api/remote-schemas#metadata-add-remote-schema) metadata API:
...
If you are running Hasura using Docker, ensure that the Hasura Docker container can reach the server endpoint. See [this page](/graphql/core/guides/docker-networking#docker-networking) for Docker networking.
...
Make a request to the [reload_remote_schema](/graphql/core/api-reference/metadata-api/remote-schemas#metadata-reload-remote-schema) metadata API.

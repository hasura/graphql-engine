...
| clean_invocation_logs | Option to indicate whether the corresponding invocation logs are also to be cleaned.                      | false   | `false` clean_invocation_logs means that invocation logs will not be cleaned.                                      |
...
| paused                | Is the auto-cleanup process paused.                                                                       | false   | `paused: true` means cleanup is paused, hence no logs will be deleted.                                             |
...
<Thumbnail src='/img/event-triggers/auto-cleanup.png' alt='Auto cleanup' max-width='533px' />

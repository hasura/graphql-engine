...
function determineScalarType(datatype: Datatype): ScalarTypeKey {
  ...
  switch (datatype.affinity) {
    ...
    default:
      console.log(
        `Unknown SQLite column type: ${datatype.variant} (affinity: ${datatype.affinity}). Interpreting as string.`,
      );
      return 'string';
  }
}
...
function getColumns(ast: any[]): ColumnInfo[] {
  return ast.map((column) => {
    const isAutoIncrement = column.definition.some(
      (def: any) => def.type === 'constraint' && def.autoIncrement === true,
    );
...
): ((info: TableInfoInternal) => TableInfo) => {
  switch (detailLevel) {
    case 'everything':
      return formatEverythingTableInfo(config);
    case 'basic_info':
      return formatBasicTableInfo(config);
    default:
...
const formatBasicTableInfo =
  (config: Config) =>
  (info: TableInfoInternal): TableInfo => {
    const tableName = config.explicit_main_schema
      ? ['main', info.name]
      : [info.name];
    return {
      name: tableName,
  ...
  (info: TableInfoInternal): TableInfo => {
    ...
    const foreignKey =
      foreignKeys.length > 0
        ? { foreign_keys: Object.fromEntries(foreignKeys) }
        : {};
...
const includeTable =
  ...
  (table: TableInfoInternal): boolean => {
    ...
    const filterForOnlyTheseTables = only_tables
      ...
      ?.filter((n) =>
        config.explicit_main_schema ? n.length === 2 && n[0] === 'main' : true,
      )
...
function ddlFKs(
  ...
  return ddl.statement.flatMap((t: any) => {
    ...
    return t.definition.flatMap((c: any) => {
      ...
      const foreignTable = config.explicit_main_schema
        ? ['main', definition.references.name]
...
function getPrimaryKeyNames(ddl: any): string[] {
  ...
  return ddl.statement
    .filter(
      (ddlStatement: any) =>
        ddlStatement.type === 'statement' &&
        ddlStatement.variant === 'create' &&
        ddlStatement.format === 'table',
    )
    .flatMap((createTableDef: any) => {
      // Try to discover PKs defined on the column
      // (eg 'Id INTEGER PRIMARY KEY NOT NULL')
      const pkColumns = createTableDef.definition
        .filter(
          (def: any) => def.type === 'definition' && def.variant === 'column',
        )
        .flatMap((columnDef: any) =>
          columnDef.definition.some(
            (def: any) =>
      ...
      const pkConstraintColumns = createTableDef.definition
        .filter(
          (def: any) =>
            def.type === 'definition' &&
            def.variant === 'constraint' &&
            def.definition.length === 1 &&
            def.definition[0].type === 'constraint' &&
            def.definition[0].variant === 'primary key',
        )
...
export async function getSchema(
  ...
  return await withConnection(config, defaultMode, sqlLogger, async (db) => {
    ...
    const result: TableInfo[] = filtered.map(
      formatTableInfo(config, detailLevel),
    );

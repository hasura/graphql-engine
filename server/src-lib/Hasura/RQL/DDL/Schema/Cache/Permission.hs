...
mkPermissionMetadataObject source table permDef =
  let permType = permAccToType (permAccessor :: PermAccessor (PermInfo a))
      objectId = MOSourceObjId source $ SMOTableObj table $
...
withPermission f = proc (e, ((source, table, permission), s)) -> do
  let metadataObject = mkPermissionMetadataObject source table permission
      permType = permAccToType (permAccessor :: PermAccessor (PermInfo c))
      roleName = _pdRole permission
      schemaObject = SOSourceObj source $ SOITableObj table $
  ...
  (| traverseA ( \permission ->
    (| withPermission (do
         ...
         (info, dependencies) <- liftEitherA <<< Inc.bindDepend -< runExceptT $
           runTableCoreCacheRT (buildPermInfo source table tableFields permission) tableCache

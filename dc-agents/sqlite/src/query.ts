  ...
  const generateWhere = (
    ...
    switch (expression.type) {
      ...
      case 'exists':
        ...
        return tag(
          'exists',
          `EXISTS (SELECT 1 FROM ${escapeTargetName(joinInfo.joinTarget)} AS ${joinInfo.joinTableAlias} WHERE ${whereComparisons})`,
        );
...
function target_query( // TODO: Rename as `target_query`
  ...
  const fieldSelect =
    fields === null ? [] : [`'rows', JSON_GROUP_ARRAY(JSON(j))`];
  const fieldFrom =
    fields === null
      ? ''
      : (() => {
  ...
  return tag(
    'table_query',
    `(SELECT JSON_OBJECT(${[...fieldSelect, ...aggregateSelect].join(', ')}) ${fieldFrom})`,
  );
}
...
function generateOrderByColumnTargetJoinInfo(
  ...
  const joinClause = tag(
    'columnTargetJoin',
    `LEFT JOIN ${escapeTargetName(relationship.target)} AS ${targetTableAlias} ON ${joinOnFragment}`,
  );
  return {
...
function generateOrderByAggregateTargetJoinInfo(
  ...
  const aggregateColumnsFragments = aggregateElements.flatMap((element) => {
    switch (element.target.type) {
      case 'column':
        return [];
      case 'star_count_aggregate':
        return `COUNT(*) AS ${getOrderByTargetAlias(element.target)}`;
      case 'single_column_aggregate':
        return `${cast_aggregate_function(element.target.function)}(${escapeIdentifier(element.target.column)}) AS ${getOrderByTargetAlias(element.target)}`;
      default:
  ...
  const whereClause = whereExpression
    ? `WHERE ${where_clause(allTableRelationships, whereExpression, relationship.target, subqueryTableAlias)}`
    : '';
...
function getOrderByTargetAlias(orderByTarget: OrderByTarget): string {
  switch (orderByTarget.type) {
    case 'column':
      return escapeIdentifier(getColumnSelector(orderByTarget.column));
    case 'star_count_aggregate':
      return escapeIdentifier('__star_count__');
    case 'single_column_aggregate':
...
function foreach_ids_table_value(
  ...
  return tag(
    ...
    `SELECT ${columns} FROM JSON_EACH(${escapeString(JSON.stringify(jsonData))})`,
  );
}
...
function foreach_query(
  ...
  return tag(
    'foreach_query',
    `WITH ${escapeTableName(foreachTableName)} AS (${foreachIdsTableValue}) SELECT ${tableSubquery} AS data`,
  );
}

...
-- |
-- Deleting an object identified by primary key.
--
...
tests = do
  it "Delete an article by its primary key" \testEnvironment -> do
          ...
          postGraphql
            ...
            [graphql|
              mutation delete_an_object {
                delete_#{schemaName}_article_by_pk (
                  id: 1
                ) {
                  id
                  title
  ...
  it "Does nothing for nonexistent primary keys" \testEnvironment -> do
    let schemaName :: Schema.SchemaName
        ...
        expected =
          [interpolateYaml|
            data:
              delete_#{schemaName}_article_by_pk: null
          |]

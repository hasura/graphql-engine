...
(
 SELECT
   ...
   CASE
     WHEN (
       (t.typtype) :: text = ('b' :: character(1)) :: text
     ) THEN 'BASE' :: text
     WHEN (
       (t.typtype) :: text = ('c' :: character(1)) :: text
     ) THEN 'COMPOSITE' :: text
     WHEN (
       (t.typtype) :: text = ('d' :: character(1)) :: text
     ) THEN 'DOMAIN' :: text
     WHEN (
       (t.typtype) :: text = ('e' :: character(1)) :: text
     ) THEN 'ENUM' :: text
     WHEN (
       (t.typtype) :: text = ('r' :: character(1)) :: text
     ) THEN 'RANGE' :: text
     WHEN (
       (t.typtype) :: text = ('p' :: character(1)) :: text
     ) THEN 'PSUEDO' :: text
     ELSE NULL :: text
   ...
   to_json(
     (
       SELECT
         array_agg(pt.typname) :: text []
       FROM
         unnest((coalesce(p.proallargtypes, p.proargtypes))) WITH ORDINALITY AS pat(oid)
         LEFT OUTER JOIN pg_catalog.pg_type pt ON (pt.oid = pat.oid)
     )
   ) AS input_arg_types,

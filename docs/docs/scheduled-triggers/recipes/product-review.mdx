...
def send_review_request():
  ...
  seven_days_ago_midnight = current_utc_date - timedelta(days=7)
  seven_days_ago_midnight = seven_days_ago_midnight.replace(hour=0, minute=0, second=0, microsecond=0)

  seven_days_ago_eod = current_utc_date - timedelta(days=7)
  seven_days_ago_eod = seven_days_ago_eod.replace(hour=23, minute=59, second=0, microsecond=0)

  seven_days_ago_midnight_string = seven_days_ago_midnight.isoformat()
  seven_days_ago_eod_string = seven_days_ago_eod.isoformat()
  ...
  query = {
    ...
    """,
    "variables": {
      "after": seven_days_ago_midnight_string,
      "before": seven_days_ago_eod_string
    }
  }
  ...
  # Create an email and send it for each order
  recipients = []
  for order in orders:
    # Create a message
    msg = MIMEMultipart()
    msg['From'] = 'sender@SuperStore.com'
    msg['To'] = f"{order['user']['name']} <{order['user']['email']}>"
    msg['Subject'] = f"We need your review, {order['user']['name'].split(' ')[0]}!"
    message_body = f"Hi {order['user']['name'].split(' ')[0]}, we hope you're enjoying your {order['product']['name']}. Please leave us a review!"
    msg.attach(MIMEText(message_body, 'plain'))

    server = create_smtp_server()
    if server is not None:
      # Send the email if the server is running
      server.sendmail('sender@SuperStore.com', order['user']['email'], msg.as_string())
      server.quit()
    else:
  ...
  return jsonify({'message': 'Review requests sent!', 'recipients': recipients})

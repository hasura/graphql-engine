...
# Cloud: Query response caching <HeadingIcon icon={`cloud`} size={36} />

## Introduction

Hasura Cloud provides support for caching query responses, in order to improve performance for queries which are
executed frequently. This includes actions and queries against remote schemas.

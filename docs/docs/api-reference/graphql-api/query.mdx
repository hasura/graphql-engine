...
<pre>
  <code>
    ...
    {` | `}
      {`{ relation-name | computed-field-name: `}
  </code>
</pre>
...
# And with relationships or computed fields:

query {
  author(where: { articles: { rating: { _gte: 4 } } }) {
    id
    name
  }
}
...
<pre>
  <code>
    {`
    { computed-field-name | relation-name: }{`
     {` | `}
      `field-name: { `}
...
# can also use relationships and computed fields:

...
Headers can be set when creating `creating <metadata_create_action>` or
`updating <metadata_update_action>` an action via the metadata API.
...
Before creating an action via the [create_action metadata API](/graphql/core/api-reference/metadata-api/actions.mdx) <metadata_create_action>`, all custom types
need to be defined via the [metadata_set_custom_types](/graphql/core/api-reference/metadata-api/custom-types.mdx) metadata API.

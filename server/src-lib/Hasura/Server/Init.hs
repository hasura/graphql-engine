...
urlRootEnv :: (String, String)
urlRootEnv =
  ( "HASURA_GRAPHQL_URL_ROOT"
  , "All APIs served at this path"
  )
...
configUnAuthRole =
  withEnvOption envVar ( long "unauthorized-role" <>
...
configUrlRoot :: ConfigM T.Text
configUrlRoot = fromMaybe "" <$>
  withEnvOption envVar ( long "url-root" <>
                         metavar "URL ROOT" <>
                         help helpDesc
                       )
  where
    (envVar, helpDesc) = urlRootEnv

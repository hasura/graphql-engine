...
You can define permissions using the `_and` operator in the `metadata > databases > [database-name] > tables > [table-name].yaml` file eg:
...
You can define column presets with either the [insert](/api-reference/syntax-defs.mdx#insertpermission) or [update](/api-reference/syntax-defs.mdx#updatepermission)
[permissions Metadata API](/api-reference/metadata-api/permission.mdx). Example using a Postgres db:

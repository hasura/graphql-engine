...
-   [Postgres: Remote schema relationships](/graphql/core/databases/postgres/schema/remote-relationships/remote-schema-relationships.mdx): To join data across tables and
    remote APIs, such as custom GraphQL servers you write, third party
    SaaS APIs, or even other Hasura instances. For example, you can join
    customer data from your tables with account data from Stripe,
    Spotify, or Auth0.
-   [Postgres: Remote source relationships](/graphql/core/databases/postgres/schema/remote-relationships/remote-source-relationships.mdx): To join data across tables between
    two different database sources, such as order information stored in
    a SQL Server database, and user information stored in a separate
    Postgres database.
-   [Postgres: Action relationships](/graphql/core/databases/postgres/schema/remote-relationships/action-relationships.mdx): To join data across tables and actions.
    For example, you can join user data from your database with the
    response from a `createUser` action, using the `id` field.
...
-   **Performance**: Hasura strives for optimal performance. It creates
    ...
    to remote schemas to avoid the [n+1 problem](https://hasura.io/learn/graphql/intro-graphql/graphql-server/)

...
ColumnExp applies to more than just the column or the `field-name` but also apply to relationships and computed fields as well.
...
query {
  articles(where: {_not: {articles: {rating: {_gte: 4}}}}) {
    title
  }
}
```

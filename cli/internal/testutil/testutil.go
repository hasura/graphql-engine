...
func StartHasuraCLIMigrations(
	t TestingT,
	image string,
	pgConnectionUrl string,
	metadataDir, migrationsDir string,
) (port string, teardown func()) {
	port, teardownHasura := StartHasuraWithPG(
		t,
		image,
		pgConnectionUrl,
		func(o *docker.HostConfig) {
			o.Binds = []string{}
...
func StartHasuraWithPG(
	...
	image string,
	pgConnectionUrl string,
	dockerOpts ...func(*docker.HostConfig),
) (port string, teardown func()) {
	if len(image) == 0 {
	...
	envs := []string{
		"HASURA_GRAPHQL_DATABASE_URL=" + pgConnectionUrl,
	...
	if len(adminSecret) > 0 {
		envs = append(envs, "HASURA_GRAPHQL_ADMIN_SECRET="+adminSecret)
	}
	...
	if err = pool.Retry(func() error {
		...
		resp, err := http.Get(
			fmt.Sprintf("http://localhost:%s/healthz", hasura.GetPort("8080/tcp")),
		)
		if err != nil {
			return err
		}
...
func StartHasuraWithMetadataDatabase(t TestingT, image string) (port string, teardown func()) {
	...
	if err = pool.Retry(func() error {
		...
		db, err = sql.Open(
			"postgres",
			fmt.Sprintf(
				"postgres://postgres:postgrespassword@%s:%s/%s?sslmode=disable",
				"0.0.0.0",
				pg.GetPort("5432/tcp"),
				"postgres",
			),
	...
	envs := []string{
		fmt.Sprintf(
			"HASURA_GRAPHQL_METADATA_DATABASE_URL=postgres://postgres:postgrespassword@%s:%s/postgres",
			DockerSwitchIP,
			pg.GetPort("5432/tcp"),
		),
		`HASURA_GRAPHQL_ENABLE_CONSOLE=true`,
...
func StartHasuraWithMSSQLSource(t *testing.T, version string) (string, string, func()) {
	...
	connectionString := fmt.Sprintf(
		"DRIVER={ODBC Driver 18 for SQL Server};SERVER=%s,%s;DATABASE=master;Uid=SA;Pwd=%s;Encrypt=optional",
		DockerSwitchIP,
		mssqlPort,
		MSSQLPassword,
	)
	AddMSSQLSourceToHasura(
		t,
		fmt.Sprintf("%s:%s", BaseURL, hasuraPort),
		connectionString,
		sourcename,
	)
...
func AddDatabaseToHasura(
	t TestingT,
	hgeEndpoint, sourceName, databaseKind string,
) (string, func()) {
	...
	if databaseKind == "mssql" {
		...
		connectionStringMSSQL := fmt.Sprintf(
			"DRIVER={ODBC Driver 18 for SQL Server};SERVER=%s,%s;DATABASE=master;Uid=SA;Pwd=%s;Encrypt=optional",
			DockerSwitchIP,
			mssqlPort,
			MSSQLPassword,
...
func AddMSSQLSourceToHasura(t TestingT, hasuraEndpoint, connectionString, sourceName string) {
	url := hasuraEndpoint + "/v1/metadata"
	body := fmt.Sprintf(`
{
...
func getUniqueName(t TestingT) string {
	return uuid.NewString() + "-" + randomdata.SillyName()
}
...
func StartCitusContainer(t TestingT) (string, func()) {
	...
	if err = pool.Retry(func() error {
		...
		db, err = sql.Open(
			"postgres",
			fmt.Sprintf(
				"postgres://postgres:test@%s:%s/%s?sslmode=disable",
				"0.0.0.0",
				pg.GetPort("5432/tcp"),
				"postgres",
			),
	...
	connectionString := fmt.Sprintf(
		"postgres://postgres:test@%s:%s/postgres",
		DockerSwitchIP,
		pg.GetPort("5432/tcp"),
	)
...
func StartHasuraWithCockroachSource(
	t TestingT,
	image string,
) (hasuraPort, sourceName string, teardown func()) {
	hasuraPort, hasuraTeardown := StartHasuraWithMetadataDatabase(t, image)
	sourceName = randomdata.SillyName()
...
func StartHasuraWithBigQuerySource(
	t TestingT,
	image string,
) (hasuraPort, sourceName, projectId, dataset string, teardown func()) {
	hasuraPort, hasuraTeardown := StartHasuraWithMetadataDatabase(t, image)
	sourceName = randomdata.SillyName()
...
func AddBigQuerySourceToHasura(
	t TestingT,
	globalSelectLimit int,
	sourceName, serviceAccount, projectId, dataset, hasuraEndpoint string,
) {
	request := fmt.Sprintf(`
    {

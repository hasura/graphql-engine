...
data ResolvedExecutionPlan
  = ResolvedPlanQuery EB.QueryExecutionPlan [IR.QueryRootField UnpreparedValue] DirectiveMap
  -- ^ query execution; remote schemas and introspection possible
  | ResolvedPlanMutation EB.MutationExecutionPlan
  -- ^ mutation execution; only __typename introspection supported
  | ResolvedPlanSubscription SubscriptionExecution
...
buildSubscriptionPlan userInfo rootFields = do
  ...
  where
    ...
    buildAction exists allFields = do
      AB.dispatchAnyBackend @EB.BackendExecute exists
        \((IR.DBField sourceName sourceConfig _) :: IR.DBField db b) -> do
           qdbs <- traverse (checkField @b sourceName) allFields
           (sourceName,) . LQP . AB.mkAnyBackend . MultiplexedLiveQueryPlan
             <$> EB.makeLiveQueryPlan userInfo sourceName sourceConfig qdbs
    ...
    checkField sourceName exists = case AB.unpackAnyBackend exists of
      ...
      Just (IR.DBField src _ (IR.QDBR qdb)) ->
        if sourceName /= src then
          throw400 NotSupported "all fields of a subscription must be from the same source"
        else pure qdb
...
getResolvedExecPlan userInfo sqlGenCtx sc queryType (reqUnparsed, reqParsed) = do
  ...
  (normalizedSelectionSet, resolvedExecPlan) <- case queryParts of
    G.TypedOperationDefinition G.OperationTypeQuery _ varDefs directives selSet -> do
      ...
      (executionPlan, queryRootFields, normalizedSelectionSet, dirMap) <-
        EQ.convertQuerySelSet gCtx userInfo directives inlinedSelSet varDefs (_grVariables reqUnparsed)
      pure $ (normalizedSelectionSet, ResolvedPlanQuery executionPlan queryRootFields dirMap)
    ...
    G.TypedOperationDefinition G.OperationTypeMutation _ varDefs directives selSet -> do
      ...
      (executionPlan, normalizedSelectionSet) <-
        EM.convertMutationSelectionSet gCtx sqlGenCtx userInfo directives inlinedSelSet varDefs (_grVariables reqUnparsed)
      pure $ (normalizedSelectionSet, ResolvedPlanMutation executionPlan)
      -- See Note [Temporarily disabling query plan caching]
      -- traverse_ (addPlanToCache . EP.RPQuery) plan
    G.TypedOperationDefinition G.OperationTypeSubscription _ varDefs directives selSet -> do
      ...
      pure (normalizedSelectionSet, ResolvedPlanSubscription subscriptionPlan)

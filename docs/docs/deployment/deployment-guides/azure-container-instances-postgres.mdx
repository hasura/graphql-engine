...
Use the `HASURA_GRAPHQL_JWT_SECRET` env variable to setup JWT. To generate a new JWT config refer [here](/auth/authentication/jwt.mdx#generating-jwt-config).
...
Check out the [Running with JWT](/auth/authentication/jwt.mdx#running-with-jwt) section for the usage of `HASURA_GRAPHQL_JWT_SECRET` env variable.

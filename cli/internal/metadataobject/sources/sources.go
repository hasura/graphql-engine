...
func (t *SourceConfig) Build() (map[string]any, error) {
	...
	return map[string]any{t.Key(): sources}, nil
}

func (t *SourceConfig) Export(metadata map[string]yaml.Node) (map[string][]byte, error) {
	var (
		op          errors.Op = "sources.SourceConfig.Export"
		sourcesNode yaml.Node
		ok          bool
	)
	...
	for idx, source := range sources {
		...
		if !source.Tables.IsZero() {
			...
			for idx, table := range tablesKey {
				...
				tableFilePath := filepath.ToSlash(
					filepath.Join(
						t.MetadataDir,
						sourcesDirectory,
						source.Name,
			...
			tableTagsFilePath := filepath.ToSlash(
				filepath.Join(
					t.MetadataDir,
					sourcesDirectory,
					source.Name,
			...
			if err := metadataobject.GetEncoder(&tableTagsBytes).Encode(tableTags); err != nil {
				return nil, errors.E(
					op,
					t.error(fmt.Errorf("building contents for %v: %w", tableTagsFilePath, err)),
				)
			}
			...
			sources[idx].Tables = yaml.Node{
				...
				Tag:   "!!str",
				Style: yaml.DoubleQuotedStyle,
				Value: "!include " + filepath.ToSlash(
					filepath.Join(source.Name, tablesDirectory, "tables.yaml"),
				),
			}
		}
		...
		if !source.Functions.IsZero() {
			...
			for idx, functionNode := range functions {
				...
				if functionNode.Function.Kind == yaml.MappingNode { // Native DB functions
					...
					err := yaml.Unmarshal(functionbs, &function)
					if err != nil {
						return nil, errors.E(op, t.error(err))
					}
					...
					functionFileName = strings.Join(function, "_") + ".yaml"
				}
				...
				functionFilePath := filepath.ToSlash(
					filepath.Join(
						t.MetadataDir,
						sourcesDirectory,
						source.Name,
			...
			if len(functions) > 0 {
				functionsTagsFilePath := filepath.ToSlash(
					filepath.Join(
						t.MetadataDir,
						sourcesDirectory,
						source.Name,
				...
				err := metadataobject.GetEncoder(&functionTagsBytes).Encode(functionTags)
				if err != nil {
					return nil, errors.E(
						op,
						t.error(
							fmt.Errorf("building contents for %v: %w", functionsTagsFilePath, err),
						),
					)
				}
				...
				sources[idx].Functions = yaml.Node{
					...
					Value: filepath.ToSlash(
						"!include " + filepath.ToSlash(
							filepath.Join(source.Name, functionsDirectory, "functions.yaml"),
						),
					),
				}
			}
...
func (t *SourceConfig) WriteDiff(opts metadataobject.WriteDiffOpts) error {
	...
	err := metadataobject.DefaultWriteDiff(
		metadataobject.DefaultWriteDiffOpts{
			From:                 t,
			WriteDiffOpts:        opts,
			ExcludeFilesPatterns: excludePatterns,
		},
	)
	if err != nil {

...
- Dynamic Routing is available only for Postgres backends currently. Coming soon for other backends.
...
Note that only non-admin roles use the connection template. Admin role will always use the `primary` connection.

There are two configurable parameters in the source configuration used for dynamic routing:

...
# List Tables
This might be a bit of complex part in migration.

This section probably needs a direct migration from RST to MDX rather than from pandoc migrated files as the auto migration messes up the code block syntax witj lot of `span`'s. Apologies for inconvinience.
...
## Docusaurus - MDX
If the table consist of simple text and no complex UI and no colum width control. This can be plain markdown syntax as below.
...
Unfortunatly, `markdown` doesn't not have straight forward syntax to control width and had to rely on html for this.
...
If a table cell is mixed with text and markdown stuff (`code highlighters`, [links](/migration-guide/links) etc., Please prefer `html` syntax over markdown as markdown wont be parsed correctly.
...
Also, start the codeblock and respective cell (`td/th`) at the very beginning of line (no empty tabs/spaces at start).
...
Please do not that this is required only when there is a cell invloving code block.

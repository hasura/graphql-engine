...
addNonColumnFields = proc ( allSources
  ...
  rawRemoteRelationshipInfos
    ...
    -< ((allSources, columns, remoteSchemaMap), map (source, _nctiTable,) _nctiRemoteRelationships)
...
mkRemoteRelationshipMetadataObject (source, table, RemoteRelationshipMetadata{..}) =
  ...
  in MetadataObject objectId
       $ toJSON
       $ RemoteRelationship @b _rrmName source table def

buildRemoteRelationship
  :: forall b arr m
   . ( ArrowChoice arr, ArrowWriter (Seq CollectedInfo) arr
     , ArrowKleisli m arr, MonadError QErr m, BackendMetadata b)
  => ( (HashMap SourceName (AB.AnyBackend SourcePartialInfo), FieldInfoMap (ColumnInfo b), RemoteSchemaMap)
     , (SourceName, TableName b, RemoteRelationshipMetadata)
...
buildRemoteRelationship = proc ( (allSources, allColumns, remoteSchemaMap)
  ...
  (| withRecordInconsistency (
       (| modifyErrA (do
          (remoteField, dependencies) <- bindErrorA -< buildRemoteFieldInfo allSources remoteRelationship allColumns remoteSchemaMap

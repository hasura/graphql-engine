...
const allInputsValidationSchema = z.object({
  ...
  selectName: z.enum(['selectValue0', 'selectValue1'], {
    message: 'Choose between Value 0 & Value 1',
  }),
  checkboxNames: z
...
export const TemplateAllInputs = args => {
  const formRef = useRef();
  useEffect(() => {
    if (args && args.triggerValidation) {
      formRef.current.trigger();
...
<Canvas>
  <Story name="Demo with all inputs">{TemplateAllInputs.bind({})}</Story>
</Canvas>

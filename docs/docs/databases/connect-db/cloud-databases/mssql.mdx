...
This guide explains how to connect a new or existing MS SQL on Azure database to a Hasura Cloud project. If
you're exploring MS SQL on Azure and are interested in migrating an existing MS SQL database - such as from
Azure - check out their
[docs](https://learn.microsoft.com/en-us/azure/dms/tutorial-sql-server-to-azure-sql) before continuing
below.
...
<Thumbnail src='/img/cloud-dbs/mssql/select-sql-server-step-1.png' alt='Navigate to SQL Servers on Azure' width='1000px' />

Click on the `Create` button:

<Thumbnail src='/img/cloud-dbs/mssql/click-on-create-step-2.png' alt='Create database in Azure' width='1000px' />

Select the current `Subscription` (Billing Account) and the resource group. Fill all the necessary fields. In the
...
Once the server instance is created, go to the server instance page and click on `Show networking settings`.

<Thumbnail src='/img/cloud-dbs/mssql/server-created-go-to-network.png'
           alt='Go to networking settings' width='1000px' />
...
Under `Firewell rules` in networking settings click on `Add a firewall rule`. Name the firewall rule and paste the Hasura
cloud IP on both Start IP and End IP input boxes:

<Thumbnail src='/img/cloud-dbs/mssql/add-ip-and-save.png' alt='Add IP to firewall' width='1000px' />

After entering the IP, click on OK and then click on `Save` at end of the page for the changes to take effect.

## Step 4: Create Database {#create-db-on-mssql-instance}

Go to the SQL Server instance home and click on `Create Database`

<Thumbnail src='/img/cloud-dbs/mssql/click-on-create-db-step-6.png' alt='Create Database on SQL Server' width='1000px' />

Fill all the required fields and additional settings if required. Click on `Review + create` and create the database.

<Thumbnail src='/img/cloud-dbs/mssql/create-db-step-7.png' alt='Review and create database' width='1000px' />

## Step 5: Construct the database connection URL {#construct-db-url-mssql}
...
- `Database`: Database name created on [step 4](#create-db-on-mssql-instance).
...
Back on Hasura Console, enter the database URL that we retrieved in [step 5](#construct-db-url-mssql):

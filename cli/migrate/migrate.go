...
func newProgressBar(str string, w io.Writer, pbLogs bool) *pb.ProgressBar { //nolint:unparam
	...
	barTemplateConfiguration := fmt.Sprintf(
		`{{ cyan %s }} {{ counters .}} {{ bar . "[" "=" ">" "." "]"}} {{percent .}}`,
		str,
	)
...
func (m *Migrate) newMigrationStatus(
	version uint64,
	driverType string,
	dirty bool,
) *MigrationStatus {
...
func (m *Migrate) UpdateSetting(name string, value string) error {
	...
	err := m.databaseDrv.UpdateSetting(name, value)
	if err != nil {
		return herrors.E(op, err)
	}
...
func (m *Migrate) ExportSchemaDump(
...
func (m *Migrate) Query(data any) error {
...
func (m *Migrate) Squash(v1 uint64, v2 int64) (vs []int64, us []byte, ds []byte, err error) {
	...
	retUp := make(chan any, m.PrefetchMigrations)
	...
	// to the returned channel
	retDown := make(chan any, m.PrefetchMigrations)
	go m.squashDown(v1, v2, retDown)

	// combine squashed up and down migrations into a single one when they're ready
	dataUp := make(chan any, m.PrefetchMigrations)
	dataDown := make(chan any, m.PrefetchMigrations)

	retVersions := make(chan int64, m.PrefetchMigrations)
	go func() {
		m.squashMigrations(retUp, retDown, dataUp, dataDown, retVersions)
	}()
...
func (m *Migrate) Migrate(version uint64, direction string) error {
	...
	if m.DryRun {
		...
		e := m.unlockErr(m.runMigrations(ret, bar))
		if e != nil {
			return herrors.E(op, e)
		}
...
func (m *Migrate) QueryWithVersion(version uint64, data io.ReadCloser, skipExecution bool) error {
	...
	if !skipExecution {
		err := m.databaseDrv.Run(data, "meta", "", false)
		if err != nil {
	...
	if version != 0 {
		err := m.databaseDrv.SetVersion(int64(version), false)
		if err != nil {
			m.databaseDrv.ResetQuery()
...
func (m *Migrate) Steps(n int64) error {
	...
	if m.DryRun {
		err := m.unlockErr(m.runDryRun(ret))
		if err != nil {
			return herrors.E(op, err)
		}
		...
		err := m.unlockErr(m.runMigrations(ret, bar))
		if err != nil {
			return herrors.E(op, err)
		}
...
func (m *Migrate) squashUp(from uint64, to int64, ret chan<- any) {
	defer close(ret)
...
func (m *Migrate) squashDown(v1 uint64, v2 int64, ret chan<- any) {
	defer close(ret)
...
func (m *Migrate) read(version uint64, direction string, ret chan<- any, bar *pb.ProgressBar) {
	defer close(ret)
...
func (m *Migrate) readUp(limit int64, ret chan<- any, bar *pb.ProgressBar) {
	defer close(ret)
...
func (m *Migrate) readDown(limit int64, ret chan<- any, bar *pb.ProgressBar) {
	defer close(ret)
...
func (m *Migrate) runMigrations(ret <-chan any, bar *pb.ProgressBar) error {
		...
		case *Migration:
			if r.Body != nil {
				if !m.SkipExecution {
					...
					err := m.databaseDrv.Run(body, r.FileType, r.FileName, noTx)
					if err != nil {
						return herrors.E(op, err)
					}
				}
				...
				err := m.databaseDrv.SetVersion(version, false)
				if err != nil {
					return herrors.E(op, err)
				}
				...
				if version != r.TargetVersion {
					...
					err := m.databaseDrv.RemoveVersion(version)
					if err != nil {
						return herrors.E(op, err)
					}
				}
...
func (m *Migrate) runDryRun(ret <-chan any) error {
...
func (m *Migrate) squashMigrations(
	retUp <-chan any,
	retDown <-chan any,
	dataUp chan<- any,
	dataDown chan<- any,
		...
		for r := range retUp {
			...
			case *Migration:
				if r.Body != nil {
					...
					err = m.databaseDrv.PushToList(r.BufferedBody, r.FileType, &squashList)
					if err != nil {
						dataUp <- err
...
func (m *Migrate) newMigration(version uint64, targetVersion int64) (*Migration, error) {
	var (
		op   herrors.Op = "migrate.Migrate.newMigration"
		migr *Migration
	)
...
func (m *Migrate) readUpFromVersion(from int64, to int64, ret chan<- any, bar *pb.ProgressBar) {
	defer close(ret)
...
func (m *Migrate) readDownFromVersion(from int64, to int64, ret chan<- any, bar *pb.ProgressBar) {
	defer close(ret)

...
FROM sys.objects object
       CROSS APPLY (SELECT [column].name, [column].column_id, [column].is_nullable, [column].system_type_id,
                    ...
                    CROSS APPLY (SELECT name, schema_id, system_type_id FROM sys.types [type]
                                 WHERE [type].system_type_id = [column].system_type_id

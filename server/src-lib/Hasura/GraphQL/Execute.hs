...
data ResolvedExecutionPlan
  = QueryExecutionPlan (EB.ExecutionPlan IR.QueryDBRoot) [IR.QueryRootField UnpreparedValue] DirectiveMap
  -- ^ query execution; remote schemas and introspection possible
  | MutationExecutionPlan (EB.ExecutionPlan IR.MutationDBRoot)

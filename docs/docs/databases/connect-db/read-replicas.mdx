...
title: 'Cloud: Read replicas on Hasura Cloud'
keywords:
  - hasura
  - docs
  - cloud
...
# Cloud: Read replicas on Hasura Cloud <HeadingIcon icon={`cloud`} size={36} />

## Introduction

Hasura Cloud can load balance queries and subscriptions across _read replicas_ while sending all mutations and metadata
API calls to the primary.

	...
	if !o.DryRun {
		if o.EC.Config.Version == cli.V2 {
			...
			if !r.IsConsistent {
				...
				o.EC.Logger.Warn(
					"Use 'hasura metadata ic list' command to list inconsistent objects",
				)
			}
...
func (m *metadataModeDirectoryHandler) Diff(o *MetadataDiffOptions) error {
	...
	if len(o.DiffType) > 0 {
		...
		err := printGeneratedMetadataFileDiffBetweenProjectDirectories(opts)
		if err != nil {
			return errors.E(op, err)
		}
	...
	err := projectmetadata.PrintContextRichDiffBetweenProjectDirectories(opts)
	if err != nil {
		return errors.E(op, err)
	}
...
func (m *metadataModeJSONHandler) Export(o *MetadataExportOptions) error {
	...
	err := export(o, o.EC.MetadataMode)
	if err != nil {
		return errors.E(op, err)
	}

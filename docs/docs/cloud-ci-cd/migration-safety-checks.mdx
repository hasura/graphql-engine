...
[pgfence](https://pgfence.dev) is an open-source CLI that analyzes your migration SQL files **before deployment** and
...
- name: Check migration safety
  run: npx --yes @flvmnt/pgfence@0.2.1 analyze --ci hasura/migrations/**/*.sql
```
...
```yaml
- name: Check migration safety
  run: npx --yes @flvmnt/pgfence@0.2.1 analyze --ci --max-risk medium hasura/migrations/**/*.sql
```
...
```yaml
- name: Check migration safety
  run: npx --yes @flvmnt/pgfence@0.2.1 analyze --output json hasura/migrations/**/*.sql > pgfence-report.json
```
...
run: npx --yes @flvmnt/pgfence@0.2.1 analyze --ci hasura/migrations/default/**/up.sql

# All data sources
run: npx --yes @flvmnt/pgfence@0.2.1 analyze --ci hasura/migrations/**/up.sql
```

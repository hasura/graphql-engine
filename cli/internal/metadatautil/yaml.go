...
const (
	baseDirectoryKey = "base_directory"
	includeTag       = "!include"
)
...
func NewYamlDecoder(opts YamlDecoderOpts, destination any) *yamlDecoder {
...
func (f *fragment) UnmarshalYAML(value *yaml.Node) error {
	var (
		op  errors.Op = "metadatautil.fragment.UnmarshalYAML"
		err error
	)
...
func (e *YamlTagResolverError) Error() string {
	if e.tag == "" {
		return "yaml tag resolver error: " + e.err.Error()
	}
...
var resolver = func(ctx map[string]string, node *yaml.Node, files *[]string) (*yaml.Node, error) {
	...
	if !ok {
		return nil, errors.E(
			op,
			&YamlTagResolverError{
				"",
				"",
				stderrors.New("parser error: base directory for !include tag not specified"),
			},
		)
	}

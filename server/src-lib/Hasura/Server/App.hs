...
type Response = (BL.ByteString, Maybe [(Text, Text)])
...
mkSpockAction qErrEncoder serverCtx handler = do
  ...
  -- log result
  logResult (Just userInfo) req reqBody serverCtx (fst <$> result) $
    Just (t1, t2)
  ...
  where
    ...
    resToResp (resp, mHdrs) = do
      uncurry setHeader jsonHeader
      onJust mHdrs $ mapM_ (uncurry setHeader)
      lazyBytes resp
...
v1QueryHandler :: RQLQuery -> Handler Response
...
v1Alpha1GQHandler :: GH.GraphQLRequest -> Handler Response
...
gqlExplainHandler :: GE.GQLExplain -> Handler Response
...
legacyQueryHandler :: TableName -> T.Text -> Handler Response

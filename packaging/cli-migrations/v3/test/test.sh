...
docker run --network container:graphql-engine appropriate/curl -s -f -r POST --header 'Content-Type: application/json' -d '{"query":"{ test { id } }"}' localhost:8080/v1/graphql | jq -j '.'  | diff validation/seeds.json -

...
func (q *QueryCollectionConfig) CreateFiles() error {
	...
	err = os.WriteFile(filepath.Join(q.MetadataDir, q.Filename()), buf.Bytes(), 0o644)
	if err != nil {
		return errors.E(op, err)
	}
...
func (q *QueryCollectionConfig) Build() (map[string]any, error) {
	...
	return map[string]any{q.Key(): obj}, nil
}
...
func (q *QueryCollectionConfig) Export(metadata map[string]yaml.Node) (map[string][]byte, error) {
	var (
		op    errors.Op = "querycollections.QueryCollectionConfig.Export"
		value any
	)
...
func (q *QueryCollectionConfig) WriteDiff(opts metadataobject.WriteDiffOpts) error {
	...
	err := metadataobject.DefaultWriteDiff(
		metadataobject.DefaultWriteDiffOpts{From: q, WriteDiffOpts: opts},
	)
	if err != nil {
...
func (q *QueryCollectionConfig) error(
	err error,

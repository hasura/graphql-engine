...
JOIN (
  ...
  FROM ( SELECT DISTINCT
    ...
    attr.attname,
    attr.conname,
    array_position(i.indkey, attr.attnum) as position
    FROM ( SELECT DISTINCT
      nr.nspname,
      r.relname,
      a.attname,
    ...
    WHERE
      ...
      AND(a.attnum = ANY (
          ...
          ELSE
            c.conkey
          END))
      AND NOT a.attisdropped) as attr, pg_index i
    WHERE
      attr.attrelid = i.indrelid
      AND i.indisprimary
      AND attr.attnum = ANY(i.indkey)
    ORDER BY position) x (tblschema, tblname, colname, cstrname)) constraint_column_usage
  ON tc.constraint_name::text = constraint_column_usage.constraint_name::text

...
2.  Add the following statement to the `down.sql` file in case you need
    to [roll back](/latest/graphql/core/migrations/advanced/rolling-back-migrations.mdx) the above statement:
...
See the `query <graphql_api_query>` API reference for the full
specifications.

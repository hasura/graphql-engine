...
onMessageHandler authMode serverEnv wsTxData wsConn rawMessage =
  ...
  where
    ...
    withUser maybeReqId f = do
      userInfoAndHeadersE <- runExceptT getUserInfoAndHeaders
      case userInfoAndHeadersE of
        Left e -> do
          logError $ EQueryError e
          sendMsg wsConn $ SMError $ ErrorMessage maybeReqId wsId $ String $ qeError e
        Right userInfoAndHeaders -> f userInfoAndHeaders
      where
        getUserInfoAndHeaders = do
    ...
    onExecute (ExecutePayload maybeReqId query) = do
      ...
      withUser (Just reqId) $ \(userInfo, headers) -> do
        logOp $ OExecute $ ExecuteQuery reqId query userInfo
        (sc, scVer) <- liftIO getSchemaCache
        execPlanE <- runExceptT $ E.getResolvedExecPlan pgExecCtx
                     planCache userInfo sqlGenCtx enableAL sc scVer manager headers query
        case execPlanE of
          Left e -> do
            logError $ EQueryError e

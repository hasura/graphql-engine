...
pgColsFromMutFld = \case
  ...
  MRet selFlds ->
    ...
    FCol (col, colTy) _ -> Just (col, colTy)
    _                   -> Nothing
...
pgColsToSelFlds cols =
  flip map cols $
  \pgColInfo -> ( fromPGCol $ pgiColumn pgColInfo
                , FCol (pgiColumn pgColInfo, pgiType pgColInfo) Nothing)
...
mkMutFldExp qt singleObj strfyNum = \case
  ...
  MRet selFlds ->
    ...
    let tabFrom = FromExpressionIdentifier $ qualTableToAliasIden qt

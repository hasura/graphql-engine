...
func (m *MigrationStateStoreHdbTable) InsertVersion(sourceName string, version int64) error {
	...
	query := hasura.PGRunSQLInput{
		...
		SQL: `INSERT INTO ` + fmt.Sprintf(
			...
			m.schema,
			m.table,
		) + ` (version, dirty) VALUES (` + strconv.FormatInt(
			version,
			10,
		) + `, ` + strconv.FormatBool(
			false,
...
func (m *MigrationStateStoreHdbTable) SetVersion(
	sourceName string,
	version int64,
	dirty bool,
) error {
	...
	if version >= 0 || (version == database.NilVersion && dirty) {
		query := hasura.PGRunSQLInput{
			...
			SQL: `INSERT INTO ` + fmt.Sprintf(
				...
				` ON CONFLICT(version) DO UPDATE SET dirty='%t'`,
				dirty,
			),
		}
...
func (m *MigrationStateStoreHdbTable) RemoveVersion(sourceName string, version int64) error {
	...
	query := hasura.PGRunSQLInput{
		Source: sourceName,
		SQL: `DELETE FROM ` + fmt.Sprintf(
			"%s.%s",
			m.schema,
			m.table,
		) + ` WHERE version = ` + strconv.FormatInt(
			version,
			10,
		),
	}
...
func (m *MigrationStateStoreHdbTable) SetVersions(
	sourceName string,
	versions []statestore.Version,
) error {
	var (
		op        errors.Op = "migrations.MigrationStateStoreHdbTable.SetVersions"
		insertSql []string
	)

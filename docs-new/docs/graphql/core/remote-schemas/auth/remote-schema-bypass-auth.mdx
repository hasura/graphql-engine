...
If you have a [webhook authorization setup](/graphql/core/auth/authentication/webhook#auth-webhooks), in the normal scenario, your authorization webhook would return `200` on success and `401` if it is either unable to authorize the current request or if the authorization information is absent (like cookie, authorization header etc.)
...
If you have a [JWT authorization setup](/graphql/core/auth/authentication/jwt#auth-jwt), to bypass the JWT auth:

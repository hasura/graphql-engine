...
const parseNumbersInNumericColumns = (schema: SchemaResponse) => {
  ...
  return (value: string, name: string): any => {
    return numericColumns.has(name) ? parseNumbers(value) : value;
  };
};
...
export const staticDataExists = async (name: string): Promise<boolean> => {
  ...
  const schemaOK = await new Promise((resolve) => {
    ...
    fs.access(schemaPath, fs.constants.R_OK, (err) =>
      err ? resolve(false) : resolve(true),
    );
  });
...
export const loadStaticData = async (name: string): Promise<StaticData> => {
  ...
  const schema: SchemaResponse = JSON.parse(
  ...
  const unzipStream = pipeline(gzipReadStream, createGunzip(), () => {});
  const xmlStr = (await streamToBuffer(unzipStream)).toString('utf16le');
  const xml = await parseStringPromise(xmlStr, {
    explicitArray: false,
    emptyTag: () => null,
    valueProcessors: [parseNumbersInNumericColumns(schema)],
  });
...
export const filterAvailableTables = (
  ...
  const data = Object.fromEntries(
    Object.entries(staticData.data).filter(([name, _]) =>
      config.tables === null ? true : config.tables.indexOf(name) >= 0,
    ),
  );
...
export const getTable = (
  ...
  const cachedTransformedData: {
    [tableName: string]: Record<string, string | number | boolean | null>[];
  } = {};

  const lookupOriginalTable = (
    tableName: string,
    ...
    switch (config.table_name_casing) {
      ...
      case 'lowercase':
        const name = Object.keys(staticData).find(
          (originalTableName) => originalTableName.toLowerCase() === tableName,
        );
        if (name == undefined)
    ...
    switch (config.column_name_casing) {
      ...
      case 'lowercase':
        return tableData.map((row) =>
          mapObject(row, ([column, value]) => [column.toLowerCase(), value]),
        );
      default:
  ...
  const lookupTable = (
    ...
    cachedTransformedData[tableName] = transformData(
      lookupOriginalTable(tableName),
    );
  ...
  return (tableName) => {
    if (config.schema) {
      ...
      return tableName.length === 1 ? lookupTable(tableName[0]) : undefined;
    }
  };
};
...
export const getSchema = (
  store: Record<string, StaticData>,
  config: Config,
  ...
  const prefixSchemaToTableName = (tableName: TableName) =>
    config.schema ? [config.schema, ...tableName] : tableName;
  ...
  const filterForOnlyTheseTables = request.filters?.only_tables
    ...
    ?.filter((n) =>
      config.schema ? n.length === 2 && n[0] === config.schema : true,
    )
  ...
  const filteredTables = schema.tables.filter((table) =>
    config.tables || filterForOnlyTheseTables
      ? (config.tables ?? [])
          ...
          .map((n) => [n])
          .find(nameEquals(table.name)) !== undefined
      : true,
  );
  ...
  const prefixedTables: TableInfo[] = filteredTables.map((table) => ({
    ...
    foreign_keys: table.foreign_keys
      ? mapObjectValues(table.foreign_keys, (constraint) => ({
          ...constraint,
          foreign_table: prefixSchemaToTableName(
            constraint.foreign_table.map(applyTableNameCasing),
          ),
          column_mapping: mapObject(
            constraint.column_mapping as Record<string, string>,
            ([outer, inner]) => [
              applyColumnNameCasing(outer),
              applyColumnNameCasing(inner),
            ],
          ),
        }))
      : table.foreign_keys,
  ...
  const filteredFunctions = (schema.functions ?? []).filter((func) =>
    filterForOnlyTheseFunctions
      ? filterForOnlyTheseFunctions
          .map((n) => [n])
          .find(nameEquals(func.name)) !== undefined
      : true,
  );

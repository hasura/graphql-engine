...
| Key         | Required | Schema                          | Description                                                                                                                                                    |
|-------------|----------|---------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| sslmode     | true     | `String`                        | The SSL connection mode. See the libpq ssl <span class="title-ref">support docs \<https://www.postgresql.org/docs/9.1/libpq-ssl.html></span> for more details. |
...
| custom_column_names | false    | [CustomColumnNames](#customcolumnnames)   | Customise the column fields                                                                                                                       |

## Custom Root Fields {#customrootfields}
...
## Custom Function Root Fields {#customfunctionrootfields}
...
| set          | false    | [ColumnPresetExp](#columnpresetexp)           | Preset values for columns that can be sourced from session variables or static values                                                                                                                                     |
...
[SameTable](#sametable) \| [RemoteTable](#remotetable)
...
{  
  "table" : [TableName](#tablename), "column" : [PGColumn](#pgcolumn)
...
{  
    "$exists" : {  
        "\_table": [TableName](#tablename), "\_where": [BoolExp](#boolexp)
    }
}
...
{  
[PGColumn](#pgcolumn) \| scalar [ComputedFieldName](#computedfieldname)
...
{  
    [PGColumn](#pgcolumn): {  
        [Operator](#operator): {  
            [PGColumn](#pgcolumn) \| \["$", [PGColumn](#pgcolumn)\]
        }
    }
}
...
-   The input value for `_st_d_within` operator is an object:
    ...
    ```
    {  
        field-name : {\_st_d\_within: {distance: Float, from: Value} }
    }
    ```
...
## RemoteSchemaName {#remoteschemname}
...
{  
"url" : url-string, "url_from_env" : env-var-string, "headers": \[ {
"name": header-name-string, "value": header-value-string,
"value_from_env": env-var-string } \], "forward_client_headers":
boolean, "timeout_seconds": integer, "customization":
...
| session_argument   | false    | <span class="title-ref">String</span>                       | Function argument which accepts session info JSON                                                                                                       |
| exposed_as         | false    | <span class="title-ref">String</span>                       | In which part of the schema should we expose this function? Either "mutation" or "query".                                                               |
...
| request_headers | false    | [TransformHeaders](#transformheaders)       | Transform headers as described here.                                                                                                               |
...
| Key           | required | Schema                               | Description                                                                                            |
|---------------|----------|--------------------------------------|--------------------------------------------------------------------------------------------------------|
| addHeaders    | false    | Object ([HeaderKey](#headerkey) : [HeaderValue](#headervalue)) | A map of Header Key Value pairs to be added to the request. Content-Type cannot be added via this map. |
| removeHeaders | false    | Array of ([HeaderKey](#headerkey))               | Headers to be removed from the request. Content-Type cannot be removed.                                |

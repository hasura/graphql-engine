	...
	for {
		...
		case tar.TypeReg:
			...
			if err != nil {
				return errors.E(
					op,
					fmt.Errorf("failed to copy %q from tar into file: %w", hdr.Name, err),
				)
			}
		default:
			return errors.E(
				op,
				fmt.Errorf("unable to handle file type %d for %q in tar", hdr.Typeflag, hdr.Name),
			)
		}
	}
...
func extractArchive(dst, fileName string, at io.ReaderAt, size int64) error {
	...
	if !ok {
		return errors.E(
			op,
			fmt.Errorf("mime type %q for archive file is not a supported archive format", t),
		)
	}

...
func (f *FunctionConfig) CreateFiles() error {
	...
	err = os.WriteFile(filepath.Join(f.MetadataDir, f.Filename()), data, 0o644)
	if err != nil {
		return errors.E(op, err)
	}
...
func (f *FunctionConfig) Build() (map[string]any, error) {
	...
	return map[string]any{f.Key(): obj}, nil
}
...
func (f *FunctionConfig) Export(metadata map[string]yaml.Node) (map[string][]byte, error) {
	...
	v, err := metadataobject.DefaultExport(
		f,
		metadata,
		f.error,
...
func (f *FunctionConfig) WriteDiff(opts metadataobject.WriteDiffOpts) error {
	...
	err := metadataobject.DefaultWriteDiff(
		metadataobject.DefaultWriteDiffOpts{From: f, WriteDiffOpts: opts},
	)
	if err != nil {

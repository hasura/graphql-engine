...
Enable the Relay API endpoint (`/v1beta1/relay`). When disabled, requests to the relay endpoint return an error with instructions to enable it. Disabling this reduces memory usage for deployments that do not use Relay.

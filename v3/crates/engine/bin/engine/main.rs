...
struct ServerOptions {
    ...
    #[arg(long, value_name = "REQUEST_BODY_SIZE_LIMIT in bytes", env = "REQUEST_BODY_SIZE_LIMIT", default_value_t = 10 * MB)]
...
async fn start_engine(server: &ServerOptions) -> Result<(), StartupError> {
    ...
    let mut app = get_base_routes(state.clone(), server.request_body_size_limit);

...
Remote schema relationships from BigQuery are supported from versions `v1.3.0` and
above.
...
`articles(id int, author_id int)` and we've added a remote schema
`author-remote-schema`.
...
3.  We set up the config to join the `id` input argument of our
    remote schema field to the `author_id` column of this table (in this
    case, the `articles` table).
...
[bigquery_create_remote_relationship](/api-reference/metadata-api/remote-relationships.mdx#metadata-bigquery-create-remote-relationship) metadata API:

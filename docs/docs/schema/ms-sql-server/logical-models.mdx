...
[Stored Procedures](/schema/ms-sql-server/stored-procedures.mdx) features to automatically create a
GraphQL API for a native database query.

You can find examples of how to use Logical Models in the
[Native Queries](/schema/ms-sql-server/native-queries.mdx) documentation. For a more detailed explanation

...
Legacy projects are restricted to a single region. If you're attempting to change regions for a legacy project, ensure
the data source is either removed or accessible from the target region. For region changes, confirm that the source
region is whitelisted and accessible from the target region.

...
-   The allow-list is stored in the metadata. To version control the
    ...
    [Managing Hasura metadata](/graphql/core/migrations/manage-metadata.mdx/) for more
    details.
...
value is* `false`). See `reference docs [command-flags](/graphql/core/deployment/graphql-engine-flags/reference.mdx#command-flags).

...
title: 'Cloud: Updating Hasura Cloud v1.3 projects to Hasura v2.0'
keywords:
  - hasura
  - docs
  - project
...
# Cloud: Updating Hasura Cloud v1.3 projects to Hasura v2.0 <HeadingIcon icon={`cloud`} size={36} />
...
Check the [core updating to v2.0 guide](/guides/upgrade-hasura-v2.mdx) and the
...
See the section on [hasura_v1_v2_compatibility](/guides/upgrade-hasura-v2.mdx#hasura-v1-v2-compatibility) to use a
...
- You can choose to unset the `HASURA_GRAPHQL_V1_BOOLEAN_NULL_COLLAPSE` env var that was added to your project during
  ...
  [See details](/guides/upgrade-hasura-v2.mdx#hasura-v2-null-where-change). We recommend moving to the new behaviour by
  unsetting the env var after verifying your project is not impacted by the change.
- Check the [Post update steps](/guides/upgrade-hasura-v2.mdx#hasura-v1-to-v2-post-update-steps) section of the core
...
<Thumbnail src='/img/graphql/cloud/projects/update-project.png' alt='Update project' width='700px' />
...
- Set `HASURA_GRAPHQL_V1_BOOLEAN_NULL_COLLAPSE` env var to `true` to maintain `v1.3` behaviour for `null` values in
  `where` filters. [(Know more)](/guides/upgrade-hasura-v2.mdx#hasura-v2-behaviour-changes)

...
SELECT r.routine_name AS function_name,
    ...
    to_json((COALESCE(p.proallargtypes, (p.proargtypes)::oid[]))::integer[]) AS input_arg_types,
    to_json(COALESCE(p.proargnames, ARRAY[]::text[])) AS input_arg_names
   FROM ((information_schema.routines r
     JOIN pg_proc p ON ((p.proname = (r.routine_name)::name)))
     JOIN pg_type t ON ((t.oid = p.prorettype)))
  WHERE (((r.routine_schema)::text !~~ 'pg_%'::text) AND ((r.routine_schema)::text <> 'information_schema'::text))
 GROUP BY r.routine_name, r.routine_schema, p.provariadic, p.provolatile, r.type_udt_schema,
          r.type_udt_name, t.typtype, p.proretset, p.proallargtypes, p.proargtypes, p.proargnames
);

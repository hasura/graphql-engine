---
title: Using Event Triggers to order-status-change notifications.
description: Succinct, tested, and reusable code recipes for common use cases in Hasura.
sidebar_label: Send a Notification when Order Status Changes
keywords:
  - hasura
  - docs
  - recipes
...
database. In this recipe, we'll create an Event Trigger that will fire whenever status of order changes. We'll then 
...
parsing the body of the request and sending the notification to the user to whom order belongs; it can be hosted anywhere, 
...
Whenever a status is changed in `orders` table, the Event Trigger fires. Hasura will send a request to the webhook
URL you provided. In this example, we're simply going to send a `POST` request. Our webhook will parse the request,
ensure the header is correct, and then send a notification to user.
...
def sendNotification(user_id, order_id, order_status):
    ...
    response = requests.post(
        ...
        """,
            "variables": {
                "user_id": user_id,
                "message": "Status of Order:${order_id} has been changed to: ${order_status}.",
            },
        },
        headers={
            "Content-Type": "application/json",
    ...
    print(f"🎉 Notification sent. The user has received the following notification: Status of Order:${orderId} has been changed to: ${orderStatus}.")
    data = response.json()
...
def order_status_change():
    ...
    sendNotification(userId, orderId, orderStatus)
...
With your server running, Hasura should be able to hit the endpoint. We can test this by updating a row with status 
value into our `orders` table. Let's do this with the following mutation from the `API` tab of the Console:
...
mutation UpdateOrder {
  update_orders(_set: {status: "Dispatched"}, 
    where: {id: {_eq: "<YOUR_ORDER_ID>"}}) {
    affected_rows
  }
}
```

We'll then see our server send notification to user to whom order belongs 🎉

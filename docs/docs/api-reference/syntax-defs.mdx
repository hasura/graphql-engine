...
`introspection_headers` is optional. When omitted, Remote Schema introspection uses `headers` for backward
compatibility. When present, introspection uses only this list; an explicit empty list sends no configured headers.
Runtime Remote Schema operations always use `headers`.

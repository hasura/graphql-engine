...
The rationale behind separating the event scheduling and event delivery mechanism
into two different threads is that the event triggers and scheduled event triggers
share a similar event delivery mechanism, while in the case of the former the new
events are generated by the DB triggers and in the latter, new scheduled events
are created using the generator thread.
...
processScheduledEvent
  ...
  if convertDuration (diffUTCTime currentTime sefScheduledTime)
    > strcToleranceSeconds stiRetryConf
    then processDead pgpool se
    else do
      let timeoutSeconds = round $ strcTimeoutSeconds stiRetryConf
...
retryOrMarkError se@ScheduledEventFull {..} err = do
  ...
  if triesExhausted && noRetryHeader
    then do
      ...
      currentTime <- liftIO getCurrentTime
      let delay = fromMaybe (round $ strcRetryIntervalSeconds sefRetryConf)
                    $ mRetryHeaderSeconds

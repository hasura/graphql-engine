...
runMonadSchema
  ...
  -> QueryContext
  -> SourceCache 'Postgres
  -> P.SchemaT (P.ParseT Identity) (ReaderT (RoleName, SourceCache 'Postgres, QueryContext, BackendSupport 'Postgres) m) a -> m a
runMonadSchema roleName queryContext pgSources m =
  flip runReaderT (roleName, pgSources, queryContext, BackendSupport () ()) $ P.runSchemaT m
...
buildMutationParser
  :: forall m n r
   . (MonadSchema n m, MonadTableInfo 'Postgres r m, MonadRole r m, Has QueryContext r, Has (BackendSupport 'Postgres) r)
  => [P.FieldParser n RemoteField]
  -> [ActionInfo 'Postgres]
  -> NonObjectTypeMap

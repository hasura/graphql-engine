...
1.  Passing `true` for the `is_enum` option of the
    [pg_track_table](/api-reference/metadata-api/table-view.mdx#metadata-pg-track-table) metadata API while tracking a table:
...
2.  Using the [pg_set_table_is_enum](/api-reference/metadata-api/table-view.mdx#metadata-pg-set-table-is-enum) metadata API to change
...
You can also [manually create migration files](/migrations-metadata-seeds/manage-migrations.mdx#create-manual-migrations) to
achieve this.
...
[manually reloading metadata](/migrations-metadata-seeds/manage-metadata.mdx#reload-metadata-manual) after inserting,

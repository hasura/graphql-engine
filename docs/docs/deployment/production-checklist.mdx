...
You should [limit the number of rows](/auth/authorization/permission-rules.mdx#limit-rows-permissions) that can be
accessed in one request, by setting the number in the select permission. This will prevent someone from accidentally or

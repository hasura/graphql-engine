...
Hasura supports nested objects. For example, you can define a type like
the following:
...
Recursive nested objects are also supported. For example, you can use
the following type:
...
up [action relationships](/graphql/core/databases/postgres/schema/remote-relationships/action-relationships.mdx) with tables/views.

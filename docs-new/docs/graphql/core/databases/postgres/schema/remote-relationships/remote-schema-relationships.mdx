...
For this example, we assume that our schema has a `users` table with the
fields `name` and `auth0_id`.
...
You can add a remote schema relationship by adding it to the
`tables.yaml` in the `metadata` directory:
...
In this example, we've added a remote schema which is a wrapper around
[Auth0](https://auth0.com/)'s REST API (see example

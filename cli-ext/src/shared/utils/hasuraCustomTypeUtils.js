...
const singularize = (kind) => kind.substr(0, kind.length - 1);

export const filterNameLessTypeLess = (arr) =>
  arr.filter((item) => !!item.name && !!item.type);

export const filterNameless = (arr) => arr.filter((item) => !!item.name);

export const filterValueLess = (arr) => arr.filter((item) => !!item.value);

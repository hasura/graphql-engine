...
Postgres as Docker containers in your local system. You can skip the steps involving connecting a database, as
you'll be applying metadata from your cloud database in a later step. Make sure that the ENV variable name for
database connection in your local setup is the same as that in your Cloud project so that your database connection
metadata will still work and not need to be updated. The value of this variable will be updated to point
to your local database. Also add any other ENV variables that you might have set on the Cloud project.
